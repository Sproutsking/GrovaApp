// ============================================================================
// src/App.jsx — v7 FINAL
//
// KEY CHANGES FROM v6:
//   • OnboardingGate wraps MainApp — handles profile setup + payment inside app
//   • MainApp reads user, profile, isAdmin, adminData from AuthContext directly
//   • loadProfileData() only fetches wallet balance + avatar (not profile/admin)
//   • No duplicate profile/admin fetches — AppGate already did that in parallel
//   • sessionManager.startSession() called once, guarded by ref
// ============================================================================
...
import AppGate, { useAuth } from "./components/Auth/AppGate";
...
function preloadTabs() {
  ...
  chunks.forEach((fn, i) =>
    setTimeout(() => fn().catch(() => {}), 1500 + i * 400),
  );
}

// ── MainApp ───────────────────────────────────────────────────────────────────
const MainApp = memo(() => {
  // ── Read from AuthContext — AppGate fetched all of this already ───────────
  const { user, profile: ctxProfile, isAdmin, adminData } = useAuth();

  // ── currentUser for display (built from context profile) ─────────────────
  const [currentUser, setCurrentUser] = useState(() => ({
    id: user?.id,
    name: ctxProfile?.full_name || "Loading…",
    username: ctxProfile?.username || "user",
    avatar: ctxProfile?.full_name?.charAt(0)?.toUpperCase() || "X",
    verified: ctxProfile?.verified || false,
    fullName: ctxProfile?.full_name || "Loading…",
  }));

  // ── Only wallet balance needs a separate fetch — profile/admin already done
  ...
  const [isSubscribed, setIsSubscribed] = useState(ctxProfile?.is_pro || false);
  ...
  // ── Session manager — start once ──────────────────────────────────────────
  useEffect(() => {
    if (!user?.id || sessionStarted.current) return;
    sessionStarted.current = true;
    setTimeout(() => sessionManager.startSession(user.id).catch(() => {}), 0);
  }, [user?.id]);
  ...
  useEffect(() => {
    ...
    // Fetch wallet balance + build avatar URL from context profile
    loadWalletAndAvatar(user.id, ctxProfile).catch(() => {});
  ...
  // ── Load wallet balance + build currentUser from context profile ──────────
  // Does NOT re-fetch profile or admin — those come from AuthContext
  const loadWalletAndAvatar = async (userId, profile) => {
    try {
      ...
      // Build avatar URL from profile already in context
      if (profile) {
        let avatarUrl = null;
        if (profile.avatar_id) {
          const base = mediaUrlService.getImageUrl(profile.avatar_id);
        ...
        setCurrentUser({
          ...
          avatar:
            avatarUrl || profile.full_name?.charAt(0)?.toUpperCase() || "X",
        ...
        setIsSubscribed(profile.is_pro || false);
      }
    } catch {
  ...
  // ── Manual refresh (pull-to-refresh or 30s desktop tick) ─────────────────
  const handleRefresh = useCallback(async () => {
    ...
    setRefreshTrigger((p) => p + 1);
    // On refresh we do re-fetch wallet + fresh avatar URL
    loadWalletAndAvatar(user.id, ctxProfile).catch(() => {});
  }, [isOnline, lastRefreshTime, user?.id, ctxProfile]); // eslint-disable-line
    ...
    try {
      ...
      if (perm === "granted") await pushService.subscribe(userId);
        ...
        setTimeout(async () => {
          ...
          if (granted) await pushService.subscribe(userId);
        }, 3000);
      }
    } catch {}
  };
  ...
  const renderContent = () => {
    ...
    const tabs = [
      ...
      },
      {
        id: "wallet",
        el: (
          // WalletView manages its own data internally — no props needed
          <Suspense fallback={<TabSkeleton />}>
            <WalletView />
          </Suspense>
        ),
...
// ── Root ──────────────────────────────────────────────────────────────────────
// AppGate    — handles authentication (login / logout)
// OnboardingGate — handles profile setup + payment (inside the app, after auth)
// MainApp    — the real app
const App = () => (
  <AppGate>
    <OnboardingGate>
      <MainApp />
