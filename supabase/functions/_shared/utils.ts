...
// Rate limiter — max `limit` calls per `windowSecs`
export async function checkRateLimit(
  ...
  if (error || !data || new Date(data.window_start) < new Date(windowStart)) {
    // Reset or create
    await db.from("p2p_rate_limits").upsert({
      user_id: userId,
      action,
      count: 1,
...
export async function audit(
  ...
  await db.from("p2p_audit_log").insert({
    actor_id: actorId,
    action,
    trade_id: tradeId,
    offer_id: offerId,
...
// Send P2P notification to a user
...
// Wallet column name by asset
export function walletColumn(asset: "XEV" | "USDT"): string {
  return asset === "XEV" ? "grova_tokens" : "usdt_balance";
}

// Validate trade status transition
const ALLOWED_TRANSITIONS: Record<string, string[]> = {
  CREATED:         ["ESCROW_LOCKED", "CANCELLED", "EXPIRED"],
  ESCROW_LOCKED:   ["PAYMENT_PENDING", "CANCELLED", "DISPUTED", "EXPIRED"],
  PAYMENT_PENDING: ["PAYMENT_SENT", "CANCELLED", "DISPUTED", "EXPIRED"],
