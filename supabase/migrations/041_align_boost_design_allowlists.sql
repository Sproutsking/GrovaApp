-- Keep the secure boost design RPCs aligned with the IDs exposed by the client catalog.
-- This migration is intentionally idempotent and only replaces validation lists.
...
begin
  ...
  select * into boost_row from public.profile_boosts
  where user_id = p_user_id and status = 'active' and expires_at > now()
  order by created_at desc limit 1 for update;
  if not found then return jsonb_build_object('success', false, 'error', 'No active boost'); end if;
  ...
  else return jsonb_build_object('success', false, 'error', 'Unsupported boost tier'); end if;
  ...
  next_selections := coalesce(boost_row.theme_selections, '{}'::jsonb) || jsonb_build_object('fontId', p_font_id, 'colorId', p_color_id);
  update public.profile_boosts set theme_selections = next_selections, updated_at = now() where id = boost_row.id;
  update public.profiles set subscription_tier = boost_row.boost_tier,
    boost_selections = coalesce(boost_selections, '{}'::jsonb) || jsonb_build_object('themeId', boost_row.active_theme_id, 'fontId', p_font_id, 'colorId', p_color_id), updated_at = now()
    where id = p_user_id;
