// ============================================================================
// src/App.jsx — v23 PUSH + CALL + NOTIFICATION FULLY WIRED
// ============================================================================
// CHANGES vs v22:
//  [APP-PUSH-1]  pushService.on("push_received") now handled in App — routes
//                incoming_call push events directly to IncomingCallToast.
//  [APP-PUSH-2]  pushService.start() called immediately (not in setTimeout)
//                so the SW bridge is active before any push arrives.
//  [APP-CALL-1]  callService.init() called before pushService.start() so the
//                user_calls channel is open when the first push arrives.
//  [APP-CALL-2]  nova:accept_call window event handler preserved.
//  [APP-CALL-3]  IncomingCallToast at root level with correct onAccept/onDecline.
//  [APP-NOTIF-1] notificationService.init() awaited before push starts so
//                badge cleared_at is loaded before any realtime events fire.
//  [APP-DM-1]   MessageNotificationService.init() called with toastRef.
//  [APP-CLEAN]  handleSignOut properly cleans up all services in order.
//  All v22 features preserved verbatim.
// ============================================================================
...
function hasOAuthCodeInUrl() {
  try {
    ...
    if (params.has("error") || params.has("error_code")) {
      ...
      ["error","error_code","error_description","state","code"].forEach(k => url.searchParams.delete(k));
...
const TabSkeleton = memo(() => (
  <div style={{ padding: "24px 16px" }}>
    {[1,2,3].map(i => (
      <div key={i} style={{
        height:"80px", background:"rgba(255,255,255,0.03)", borderRadius:"12px",
        marginBottom:"12px", animation:"skPulse 1.4s ease-in-out infinite",
...
const OfflineBanner = memo(({ visible }) => {
  ...
  return (
    <div style={{
      ...
      gap:"8px", backdropFilter:"blur(4px)",
    }}>
...
// ═════════════════════════════════════════════════════════════════════════════
const MainApp = memo(() => {
  const { user, profile, isAdmin, adminData, signOut } = useAuth(); // eslint-disable-line
  ...
  // ── Resize ──────────────────────────────────────────────────────────────
  useEffect(() => {
    const onResize = () => { const m = checkMobile(); setIsMobile(m); if (m) setShowAdminDashboard(false); };
    onResize();
  ...
  const handleNotificationNavigate = useCallback((path) => {
    ...
    if (path === "/messages" || path.startsWith("/messages")) {
      setShowMessages(true); setDmTargetUserId(null); return;
    }
    ...
    if (postMatch) {
      ...
      setActiveTab("account"); setMountedTabs(p => new Set([...p, "account"]));
    }
    setShowAdminDashboard(false); setOverlayTab(null);
  }, [user?.id]);

  // ── Init — [APP-CALL-1] callService FIRST, then notifService, then push ──
  useEffect(() => {
    if (initDone.current || !user?.id || !profile) return;
    initDone.current = true;

    // [APP-CALL-1] Open user_calls channel immediately
    callService.init(user.id, profile.full_name || "User", profile.avatar_id || null);

    // [APP-NOTIF-1] Init notification service (loads badge cleared_at)
    notificationService.init(user.id).catch(() => {});

    // [APP-DM-1] Init DM notification service with toast callback
    ...
    setTimeout(() => {
      try { supabase.rpc("end_my_live_sessions", { p_user_id: user.id }).then(() => {}).catch(() => {}); } catch {}
    }, 3000);

    return () => clearTimeout(refreshTimeout.current);
  }, [user?.id, profile]); // eslint-disable-line

  // [APP-PUSH-1] Route incoming_call pushes to the call toast
  useEffect(() => {
    const unsub = pushService.on("push_received", (payload) => {
      const type = payload?.data?.type;
      if (type === "incoming_call") {
        // callService will get this via its own channel subscription
        // This is just a belt+suspenders fallback
        ...
        callService.emit("incoming_call", {
          ...
          callerName:     data.caller_name        || data.callerName || "Caller",
          callerAvatarId: data.caller_avatar_id   || null,
          caller: {
            id:        data.caller_id  || null,
            full_name: data.caller_name || "Caller",
            avatar_id: data.caller_avatar_id || null,
          },
        });
      }
    });
  ...
  // [APP-CALL-2] nova:accept_call window event
  useEffect(() => {
    const handler = (e) => {
  ...
  // [APP-CALL-5] OS push notification accept/decline
  useEffect(() => {
    ...
    // Cold open: /messages?accept_call=<callId>
    const params = new URLSearchParams(window.location.search);
    const coldCallId = params.get("accept_call");
    if (coldCallId) {
      const url = new URL(window.location.href);
  ...
  useEffect(() => {
    if (!user?.id) return;
    const unsub1 = pushService.on("notification_clicked", ({ url }) => {
      if (url) handleNotificationNavigate(url);
    });
    const unsub2 = pushService.on("sw_updated", ({ version }) => {
  ...
  useEffect(() => {
    ...
    const onVisible = async () => {
      ...
      try {
        ...
        if (error) { console.warn("[Session] refresh failed:", error.message); return; }
  ...
  const loadWalletAndAvatar = async (userId, p) => {
    try {
      ...
      if (p) {
        ...
        // Re-init callService with latest avatar
  ...
  // [APP-CLEAN] Sign out — clean up in correct order
  const handleSignOut = useCallback(async () => {
    try {
      if (user?.id) {
        // Stop incoming calls first
        callService.cleanup();
        // Then stop notifications
        notificationService.destroy();
        MessageNotificationService.cleanup();
        // Then unsubscribe push (last — needs user.id)
  ...
  const handleJoinStream = useCallback((session) => {
    setStreamSession(session); setOverlayTab("stream");
  }, []);
  ...
  const handleTabChange = useCallback((newTab) => {
    ...
    if (newTab === "admin") { if (isAdmin) setShowAdminDashboard(true); return; }
    if (OVERLAY_TABS.has(newTab)) { if (newTab !== "stream") setStreamSession(null); setOverlayTab(newTab); return; }
    if (newTab !== "home") setFeedFilter(null);
    setOverlayTab(null); setActiveTab(newTab); setShowAdminDashboard(false);
    setMountedTabs(p => { if (p.has(newTab)) return p; return new Set([...p, newTab]); });
  }, [isAdmin, isMobile]);

  const closeOverlayToAccount = useCallback(() => {
    setOverlayTab(null); setStreamSession(null); setActiveTab("account"); setMountedTabs(p => new Set([...p, "account"]));
  }, []);
  const closeOverlayToHome = useCallback(() => {
    setOverlayTab(null); setStreamSession(null); setActiveTab("home"); setMountedTabs(p => new Set([...p, "home"]));
  }, []);
  ...
  const renderContent = () => {
    const tabs = [
      { id:"home",      el:<Suspense fallback={<TabSkeleton/>}><div ref={feedRef}><HomeView {...viewProps} homeSection={homeSection} setHomeSection={setHomeSection} feedFilter={feedFilter} onClearFilter={() => setFeedFilter(null)} onJoinStream={handleJoinStream} activeHomeTab={activeHomeTab} setActiveHomeTab={setActiveHomeTab}/></div></Suspense> },
  ...
  const renderOverlay = () => {
    ...
    switch (overlayTab) {
      ...
      case "stream":    return <Suspense fallback={null}><StreamView     currentUser={currentUser} userId={user.id} onClose={closeOverlayToHome} streamSession={streamSession}/></Suspense>;
  ...
  const renderSidebar = () => {
    ...
    if (isAdmin) return <AdminSidebar activeTab={activeTab} setActiveTab={handleTabChange} sidebarOpen={sidebarOpen} setSidebarOpen={setSidebarOpen} onSignOut={handleSignOut} user={user} adminData={adminData} onOpenDashboard={() => setShowAdminDashboard(true)}/>;
  ...
  return (
    <div className="app-container">
      ...
      <div style={{ visibility:showAdminDashboard?"hidden":"visible", pointerEvents:showAdminDashboard?"none":"auto", display:"contents" }}>
        {!isMobile && (
          <DesktopHeader
            activeTab={activeTab} setActiveTab={handleTabChange} userBalance={userBalance}
            currentUser={currentUser} getGreeting={getGreeting} setSidebarOpen={setSidebarOpen}
            onNotificationClick={() => setShowNotifications(true)} onSupportClick={() => setShowSupport(true)}
            profile={profileData} userId={user?.id} onSignOut={handleSignOut}
            activeHomeTab={activeHomeTab} setActiveHomeTab={setActiveHomeTab}
          />
        )}
        {isMobile && (
          <MobileHeader
            userBalance={userBalance} getGreeting={getGreeting} setActiveTab={handleTabChange}
            onNotificationClick={() => setShowNotifications(true)} onSupportClick={() => setShowSupport(true)}
            profile={profileData} userId={user?.id} currentUser={currentUser} onSignOut={handleSignOut}
            activeTab={activeTab} activeHomeTab={activeHomeTab} setActiveHomeTab={setActiveHomeTab}
          />
        )}
        ...
        {!isMobile && (
          <div className="desktop-layout">
            ...
            <main ref={containerRef} className="main-content-desktop">{renderContent()}</main>
            {showTrending && (
              <Suspense fallback={null}>
                <TrendingSidebar currentUser={currentUser} isMobile={false} setActiveTab={handleTabChange} setFeedFilter={setFeedFilter} onJoinStream={handleJoinStream}/>
              </Suspense>
            )}
          </div>
        )}
      ...
      {showExitPrompt && (
        <div style={{
          position:"fixed", bottom:isMobile?"68px":"20px", left:"50%", transform:"translateX(-50%)",
          background:"rgba(0,0,0,0.9)", color:"#84cc16", padding:"12px 24px",
          borderRadius:"8px", fontSize:"14px", fontWeight:"600", zIndex:10000,
          border:"1px solid #84cc16", animation:"xSlideUp .3s ease-out",
        }}>
      ...
      <NotificationSidebar
        ...
        userId={user?.id} currentUser={currentUser} onNavigate={handleNotificationNavigate}
      />
      ...
      {/* [APP-CALL-3] IncomingCallToast — always mounted at root */}
      <IncomingCallToast
        onAccept={(callData) => {
          setAcceptedCallData({ ...callData, outgoing: false });
          setShowActiveCall(true);
        }}
        onDecline={(callId) => callService.declineCall(callId)}
      />
      ...
      {showOfflineBanner && (
        <NetworkError onRetry={() => { setShowOfflineBanner(false); handleRefresh(); }}/>
      )}

      <style>{`
        @keyframes xSlideUp { from{transform:translateX(-50%) translateY(20px);opacity:0} to{transform:translateX(-50%) translateY(0);opacity:1} }
...
function AppRouter() {
  ...
  useEffect(() => {
    if (!loading) return;
    const t = setTimeout(() => { console.warn("[AppRouter] Auth timeout (10s)"); setForceResolve(true); }, 10_000);
    return () => clearTimeout(t);
  }, [loading]);

  useEffect(() => {
    if (!user || profile) return;
    const t = setTimeout(() => { console.warn("[AppRouter] Profile timeout (15s)"); setProfileTimedOut(true); }, 15_000);
    return () => clearTimeout(t);
  }, [user, profile]);

  useEffect(() => { if (profile) setProfileTimedOut(false); }, [profile]);
  ...
  if (!profile) {
    const paid = getIsPaidCached?.() || false;
    if (isAdmin || paid) return <MainApp/>;
    return <Splash/>;
  }
  const paid = getIsPaidCached?.() || false;
  if (canAccessApp({ profile, isAdmin, adminData, paidCache: paid })) return <MainApp/>;
