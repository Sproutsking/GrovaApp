...
export default function ToolsSection({ communityId, userId, channels = [], canManage = false, onOpenInvite, onOpenUpgrade, onOpenModeration, onCreateChannel }) {
  ...
  const saveVerificationConfig = async (config) => {
    if (!canManage || !dashboardTool) return;
  ...
  const saveModerationConfig = async (config) => {
    if (!canManage || !dashboardTool) return;
    const row = getRow("moderation");
    const nextRow = { ...row, community_id: communityId, tool_type: "moderation", enabled: config.enabled !== false, channel_id: row.channel_id || null, config, updated_at: new Date().toISOString() };
    const { error: saveError } = await supabase.from("community_tool_settings").upsert(nextRow, { onConflict: "community_id,tool_type" });
    if (saveError) throw saveError;
    setRows((current) => [...current.filter((item) => item.tool_type !== "moderation"), nextRow]);
    setDashboardTool(null);
  };
  ...
  return (
    <section className="community-tools-section">
      ...
      {dashboardTool && dashboardTool.type === "verification" && ReactDOM.createPortal(<div className="tool-dashboard-overlay" style={{ zIndex: 100002 }} onClick={() => { setDashboardTool(null); setOpenTool(null); setFocusedVerificationMode(null); }}><section className="tool-dashboard" onClick={(event) => event.stopPropagation()}><header><div><span className="tool-dashboard-kicker">Community tool dashboard</span><h2>{focusedVerificationMode === "quick" ? "Quick verify" : focusedVerificationMode === "picture" ? "Picture check" : focusedVerificationMode === "rules_gate" ? "Rules gate" : focusedVerificationMode === "role_grant" ? "Role grant" : "Verification"}</h2><p>{focusedVerificationMode ? "Configure this verification flow in its own section." : "Choose the verification method to configure, then return to the tools screen."}</p></div><div className="tool-dashboard-header-actions"><button type="button" className="tool-dashboard-back" onClick={() => { if (focusedVerificationMode) { setFocusedVerificationMode(null); return; } setDashboardTool(null); setOpenTool(null); setFocusedVerificationMode(null); }} aria-label={focusedVerificationMode ? "Back to verification" : "Back to tools"}>{focusedVerificationMode ? "Back to verification" : "Back to tools"}</button><button type="button" onClick={() => { setDashboardTool(null); setOpenTool(null); setFocusedVerificationMode(null); }} aria-label="Close verification dashboard"><X size={18} /></button></div></header><div className="tool-dashboard-body"><VerificationToolDashboard value={getRow("verification").config} focusMode={focusedVerificationMode} channels={channels} disabled={!canManage} onSave={saveVerificationConfig} /></div></section></div>, document.body)}
