...
import { ArrowDown, ArrowUp, Check, FolderPlus, Pencil, Shield, Trash2, X } from "lucide-react";
...
export default function CategoryManagementSection({ communityId, onChanged }) {
  ...
  return (
    <section className="category-management-section">
      ...
      {loading ? <p className="category-management-muted">Loading categories...</p> : categories.map((category, index) => <div className="category-management-row" key={category.id}>{editingCategory?.id === category.id ? <><input className="category-management-edit-input" value={editingName} onChange={(event) => setEditingName(event.target.value)} maxLength={50} autoFocus /><button type="button" title="Save category" disabled={saving || !editingName.trim()} onClick={() => rename(category)}><Check size={14} /></button><button type="button" title="Cancel" disabled={saving} onClick={() => { setEditingCategory(null); setEditingName(""); }}><X size={14} /></button></> : <><span className="category-management-name">{category.name}</span><button type="button" title="Manage category permissions" disabled={saving} onClick={() => setPermissionCategory(category)}><Shield size={14} /></button><button type="button" title="Move up" disabled={index === 0 || saving} onClick={() => move(index, -1)}><ArrowUp size={14} /></button><button type="button" title="Move down" disabled={index === categories.length - 1 || saving} onClick={() => move(index, 1)}><ArrowDown size={14} /></button><button type="button" title="Edit category" disabled={saving} onClick={() => { setEditingCategory(category); setEditingName(category.name); }}><Pencil size={14} /></button><button type="button" title="Remove" disabled={category.name === "Welcome" || saving} onClick={() => remove(category)}><Trash2 size={14} /></button></>}</div>)}
