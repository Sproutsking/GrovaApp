...
export default function WelcomeMemberCard({ communityId, memberId, community, createdAt, onProfileClick, onIntroduce, onBrowse }) {
  ...
  return <article className={`welcome-member-card welcome-member-card-${config.layout}`} style={{ "--welcome-accent": theme.accent, "--welcome-bg": theme.bg, "--welcome-border": theme.border, "--member-role-color": roleColor }}>
    ...
    <div className="welcome-member-main"><div className="welcome-member-copy"><div className="welcome-member-kicker">Welcome to {community?.name || "the community"}</div><button type="button" className="welcome-member-name" onClick={() => onProfileClick?.(member)} style={{ color: design.color?.color || roleColor, fontFamily: design.font?.family, fontWeight: design.font?.weight }}>{name}</button><p>{config.description}</p><div className="welcome-member-meta">{role && <span style={{ color: roleColor, borderColor: `${roleColor}66` }}><ShieldCheck size={12} /> {role.name}</span>}{member?.verified && <span><Check size={12} /> Verified</span>}{member?.subscription_tier && <span>{member.subscription_tier} boost</span>}</div>{config.showFeatures && <div className="welcome-member-features">{features.map((feature) => <span key={feature}>{feature}</span>)}</div>}</div><button type="button" className="welcome-member-avatar" onClick={() => onProfileClick?.(member)} aria-label={`Open ${name}'s profile`}><BoostAvatarRing tier={member?.subscription_tier} themeId={member?.boost_selections?.themeId} accentColor={design.color?.color || roleColor} size={92} src={avatar} letter={name.charAt(0).toUpperCase()} showBadge /></button></div>
