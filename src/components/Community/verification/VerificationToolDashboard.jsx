...
export default function VerificationToolDashboard({ value, onSave, disabled = false, focusMode = null }) {
  ...
  if (!selectedMode) {
    const modes = [
      ...
      ["rules_gate", "Rules gate", "Require members to accept your community rules before they can unlock the role.", true],
  ...
  return (
    <div className="verification-dashboard">
      ...
      <section><h3>Verification modes</h3><div className="verification-mode-row"><button type="button" className={config.mode === "quick" ? "selected" : ""} onClick={() => patch({ mode: "quick" })}>Quick verify</button><button type="button" className={config.mode === "picture" ? "selected" : ""} onClick={() => patch({ mode: "picture" })}>Picture check</button></div><label className="verification-check"><input type="checkbox" checked={config.quickEnabled} disabled={disabled} onChange={(event) => patch({ quickEnabled: event.target.checked })} /> Quick verify enabled</label><label className="verification-check"><input type="checkbox" checked={config.pictureEnabled} disabled={disabled} onChange={(event) => patch({ pictureEnabled: event.target.checked })} /> Picture check enabled</label><label className="verification-check"><input type="checkbox" checked={config.rulesEnabled} disabled={disabled} onChange={(event) => patch({ rulesEnabled: event.target.checked })} /> Rules gate enabled</label></section>
      <section><h3>Role grant</h3><label className="verification-check"><input type="checkbox" checked={config.roleGrant?.enabled} disabled={disabled} onChange={(event) => patch({ roleGrant: { ...config.roleGrant, enabled: event.target.checked } })} /> Grant a role after successful verification</label><label>Role name<input value={config.roleGrant?.roleName || "Verified"} disabled={disabled || !config.roleGrant?.enabled} onChange={(event) => patch({ roleGrant: { ...config.roleGrant, roleName: event.target.value } })} /></label><label>Role ID or reference<input value={config.roleGrant?.roleId || ""} disabled={disabled || !config.roleGrant?.enabled} onChange={(event) => patch({ roleGrant: { ...config.roleGrant, roleId: event.target.value } })} placeholder="Optional role reference" /></label></section>
