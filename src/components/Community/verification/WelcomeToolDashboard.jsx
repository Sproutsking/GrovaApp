...
export default function WelcomeToolDashboard({ value, communityName, channels = [], disabled = false, onSave }) {
  ...
  return (
    <div className="welcome-builder">
      ...
      <div className="welcome-builder-block"><strong>Words members see</strong><label>Eyebrow<input disabled={disabled} value={config.eyebrow} maxLength={60} onChange={(event) => update({ eyebrow: event.target.value })} /></label><label>Headline<input disabled={disabled} value={config.title} maxLength={150} onChange={(event) => update({ title: event.target.value })} /></label><label>Description<textarea disabled={disabled} value={config.description} maxLength={500} rows={3} onChange={(event) => update({ description: event.target.value })} /></label><div className="welcome-builder-two-col"><label>Primary action<input disabled={disabled} value={config.primaryLabel} maxLength={32} onChange={(event) => update({ primaryLabel: event.target.value })} /></label><label>Secondary action<input disabled={disabled} value={config.secondaryLabel} maxLength={32} onChange={(event) => update({ secondaryLabel: event.target.value })} /></label></div><label>Introduce yourself opens<select disabled={disabled} value={config.introChannelId} onChange={(event) => update({ introChannelId: event.target.value })}><option value="">Select a channel</option>{channels.filter((channel) => channel.type !== "voice").map((channel) => <option key={channel.id} value={channel.id}>#{channel.name}</option>)}</select></label><small className="welcome-builder-help">Select or create a channel before activating this welcome experience.</small><label className="welcome-builder-toggle"><input type="checkbox" disabled={disabled} checked={config.showPrimaryAction} onChange={(event) => update({ showPrimaryAction: event.target.checked })} /> Show Introduce yourself</label><label className="welcome-builder-toggle"><input type="checkbox" disabled={disabled} checked={config.showSecondaryAction} onChange={(event) => update({ showSecondaryAction: event.target.checked })} /> Show Browse channels</label></div>
      {onSave && <button type="button" className="welcome-builder-save" disabled={disabled || (config.showPrimaryAction && !config.introChannelId)} onClick={() => onSave(normalizeWelcomeConfig(config))}><ExternalLink size={15} /> Save welcome experience</button>}
