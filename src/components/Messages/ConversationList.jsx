...
// Only addition: `hideHeader` prop — when true, the built-in header is not rendered
// because DMMessagesView owns the shared hub header.
...
const ConversationList = ({
  ...
  hideHeader, // NEW — pass true when parent owns the header
}) => {
  ...
  return (
    <div className="conv-root">
      {/* ── Header — only rendered when NOT hidden by parent ── */}
      {!hideHeader && (
      ...
      <style>{`
        ...
        /* ── Header (only shown when hideHeader is false) ── */
        .conv-header {
          display: flex; align-items: center; justify-content: space-between;
          padding: 14px 16px 10px;
          border-bottom: 1px solid rgba(132, 204, 22, 0.1); flex-shrink: 0;
        }
        .conv-title { font-size: 18px; font-weight: 800; color: #fff; margin: 0; flex: 1; text-align: center; letter-spacing: -0.3px; }
        .conv-close-full {
          width: 36px; height: 36px; border-radius: 10px;
          background: rgba(255, 255, 255, 0.04); border: 1px solid rgba(255, 255, 255, 0.08);
          display: flex; align-items: center; justify-content: center;
          color: #84cc16; cursor: pointer; transition: background 0.2s;
        }
        ...
        .conv-search input {
          ...
          font-size: 14px; outline: none; caret-color: #84cc16;
        }
        .conv-search input::placeholder { color: #444; }
        .conv-search-clear { background: none; border: none; color: #555; cursor: pointer; display: flex; align-items: center; }

        /* ── List scroll ── */
        .conv-list { flex: 1; overflow-y: auto; -webkit-overflow-scrolling: touch; padding-bottom: env(safe-area-inset-bottom, 16px); }
        .conv-list::-webkit-scrollbar { width: 3px; }
        .conv-list::-webkit-scrollbar-thumb { background: rgba(132, 204, 22, 0.2); border-radius: 2px; }

        /* ── Loading ── */
        .conv-loading { display: flex; justify-content: center; align-items: center; padding: 60px 20px; }
        .conv-spinner {
          width: 28px; height: 28px;
          border: 3px solid rgba(132, 204, 22, 0.15); border-top-color: #84cc16;
          border-radius: 50%; animation: convSpin 0.7s linear infinite;
        }
        @keyframes convSpin { to { transform: rotate(360deg); } }

        /* ── Empty ── */
        .conv-empty { display: flex; flex-direction: column; align-items: center; padding: 60px 20px; gap: 10px; color: #444; text-align: center; }
        ...
        .conv-start-btn {
          ...
          color: #84cc16; font-size: 13px; font-weight: 700; cursor: pointer; transition: background 0.2s;
        }
        ...
        .conv-avatar {
          ...
          display: flex; align-items: center; justify-content: center;
          font-size: 18px; font-weight: 700; color: #84cc16; overflow: hidden;
        }
        .conv-avatar img { width: 100%; height: 100%; object-fit: cover; }
        .conv-dot {
          position: absolute; bottom: 1px; right: 1px;
          width: 12px; height: 12px; border-radius: 50%;
          border: 2px solid #000; background: #333; transition: background 0.3s;
        }
        ...
        .conv-name { font-size: 14px; font-weight: 700; color: #fff; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; flex: 1; min-width: 0; }
        .conv-time { font-size: 11px; color: #444; flex-shrink: 0; margin-left: 8px; }
        .conv-bottom { display: flex; align-items: center; justify-content: space-between; gap: 8px; }
        .conv-preview { font-size: 13px; color: #4a4a4a; flex: 1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-weight: 400; }
        ...
        .conv-badge {
          ...
          background: #84cc16; color: #000; font-size: 10px; font-weight: 800;
