// ============================================================================
// components/Messages/ActiveCall.jsx — NOVA CALL ENGINE v6 FINAL
// ============================================================================
// FIXED vs v5:
//  [FIX-CALLEE-STAGE] When the callee opens ActiveCall from IncomingCallToast,
//                     stage must start as "in" (not "out"). Previously the
//                     callee was passed call.outgoing=undefined so stage
//                     defaulted to "out" (ringing) — callee saw "Calling…"
//                     instead of the Accept/Decline buttons.
//                     Now: call.outgoing===true → "out", anything else → "in".
//
//  [FIX-INVITE-CH]    sendInvites now calls MessageNotificationService which
//                     sends to "incoming_calls:{calleeId}" — exactly the
//                     channel the callee's service is subscribed to.
//
//  [FIX-NORM]         currentUser normalised once to localUser with all
//                     avatar/name fields resolved so Av component always works.
//
//  [FIX-ACCEPT-NAV]   When callee accepts via IncomingCallToast (outside DM
//                     panel), App.jsx opens ActiveCall with the call info.
//                     The callId is passed through so both sides join the
//                     SAME signalling channel "call:{callId}".
// ============================================================================
...
const PRESETS = [
  { id: "whisper", label: "Whisper", icon: "🍃", color: "#22c55e", est: "~45 KB/min",  constraints: { audio: { sampleRate: 8000,  echoCancellation: true, noiseSuppression: true }, video: false } },
  { id: "crystal", label: "Crystal", icon: "💎", color: "#84cc16", est: "~180 KB/min", constraints: { audio: { sampleRate: 24000, echoCancellation: true, noiseSuppression: true }, video: false } },
  { id: "vision",  label: "Vision",  icon: "👁️", color: "#60a5fa", est: "~1.4 MB/min", constraints: { audio: { sampleRate: 48000, echoCancellation: true }, video: { width: { ideal: 640 },  height: { ideal: 360 }, frameRate: { ideal: 15 } } } },
  { id: "vivid",   label: "Vivid",   icon: "✨", color: "#c084fc", est: "~3.2 MB/min", constraints: { audio: { sampleRate: 48000, echoCancellation: true }, video: { width: { ideal: 1280 }, height: { ideal: 720 }, frameRate: { ideal: 30 } } } },
];
...
const EndScreen = ({ reason, call, onClose, onCallback }) => {
  ...
  return (
    <div className="es-root" style={{ background: bg }}>
      <div className="es-particles">{[0, 1, 2].map(i => <div key={i} className={`es-p es-p${i}`} style={{ background: c.color }}/>)}</div>
      ...
      {reason === "ended" && call.duration > 0 && <div className="es-dur" style={{ color: c.color }}>Duration: {fmt(call.duration)}</div>}
      <div className="es-actions">
        {c.actions.includes("callback") && <button className="es-btn" style={{ background: `${c.color}18`, borderColor: `${c.color}44`, color: c.color }} onClick={() => onCallback?.("callback")}><Ic.Phone/><span>Call again</span></button>}
        {c.actions.includes("retry")    && <button className="es-btn" style={{ background: `${c.color}18`, borderColor: `${c.color}44`, color: c.color }} onClick={() => onCallback?.("retry")}><Ic.Retry/><span>Try again</span></button>}
        {c.actions.includes("message")  && <button className="es-btn es-btn-msg" onClick={() => { onCallback?.("message"); onClose(); }}><Ic.Msg/><span>Send message</span></button>}
        <button className="es-btn es-btn-close" onClick={onClose}><Ic.Close/><span>Close</span></button>
      </div>
    </div>
  );
};
...
const PTile = memo(({ p, speak, onExp }) => {
  ...
  return (
    <div className={`ptile${speak ? " ptile-speak" : ""}${hasVid ? "" : " ptile-novc"}`} onClick={() => onExp?.(p)}>
      {hasVid && <video ref={vRef} autoPlay playsInline muted={p.isLocal} className="ptile-vid" style={{ transform: p.isLocal ? "scaleX(-1)" : "none" }}/>}
      {!hasVid && (
        <div className="ptile-center">
          {url ? <img src={url} alt={name} className="ptile-avimg" onError={() => setImgErr(true)}/> : <span className="ptile-avinit">{ini(p.full_name || p.name)}</span>}
    ...
    <div className="exm-ov" onClick={onClose}>
      <div className={`exm-modal${mobile ? " exm-mobile" : ""}`} onClick={e => e.stopPropagation()}>
        ...
        {hasVid
          ...
          : <div className="exm-avbg">{url ? <img src={url} alt={name} className="exm-avimg" onError={() => setImgErr(true)}/> : <span className="exm-avinit">{ini(p.full_name || p.name)}</span>}</div>
        }
...
const ActiveCall = ({ call, onEnd, currentUser }) => {
  ...
  // [FIX-CALLEE-STAGE] Caller sees "out" (ringing), callee sees "in" (accept/decline)
  const initialStage = call.outgoing === true ? "out" : "in";

  // Normalise currentUser
  const localUser = {
    id:        currentUser?.id,
    full_name: currentUser?.fullName || currentUser?.full_name || currentUser?.name || "You",
    name:      currentUser?.fullName || currentUser?.full_name || currentUser?.name || "You",
  ...
  const destroyPC = useCallback(() => {
    try { pcR.current?.close(); } catch (_) {}
    pcR.current = null;
    lsR.current?.getTracks().forEach(t => { try { t.stop(); } catch (_) {} });
    lsR.current = null; asR.current = null; vsR.current = null;
  ...
  const buildPC = useCallback(() => {
    ...
    pc.onconnectionstatechange = () => {
      ...
      if (s === "connected") setStage("live");
      if (s === "disconnected") { try { pc.restartIce(); } catch (_) {} }
      if (s === "failed") { setEndR("failed"); setStage("ended"); }
    };
    return pc;
  }, []);
  ...
  const addTracks = useCallback((pc, stream) => {
    ...
    stream.getTracks().forEach(t => {
      try { const s = pc.addTrack(t, stream); if (t.kind === "audio") asR.current = s; if (t.kind === "video") vsR.current = s; }
      catch (e) { console.error("[Call] addTrack:", e); }
    });
  }, []);

  const flushIce = useCallback(async pc => {
    for (const c of pIce.current) { try { await pc.addIceCandidate(new RTCIceCandidate(c)); } catch (_) {} }
    pIce.current = [];
  }, []);
  ...
  const buildCh = useCallback((callId) => {
    const ch = supabase
      ...
      .on("broadcast", { event: "call_accepted" }, async () => {
        if (!mR.current) return;
        setStage("conn");
        const pc = pcR.current;
        if (!pc || pc.signalingState !== "stable") return;
        try {
          const o = await pc.createOffer({ offerToReceiveAudio: true, offerToReceiveVideo: isVid });
          await pc.setLocalDescription(o);
          ch.send({ type: "broadcast", event: "offer", payload: { sdp: o } });
        } catch (e) { console.error("[Call] offer:", e); }
      })
      .on("broadcast", { event: "call_declined" }, () => {
        if (!mR.current) return; setEndR("declined"); setStage("ended"); destroyPC();
      })
      .on("broadcast", { event: "offer" }, async ({ payload }) => {
        ...
        try {
          ...
          ch.send({ type: "broadcast", event: "answer", payload: { sdp: a } });
      ...
      .on("broadcast", { event: "answer" }, async ({ payload }) => {
        ...
        try { await pc.setRemoteDescription(new RTCSessionDescription(payload.sdp)); await flushIce(pc); }
        catch (e) { console.error("[Call] setRemote:", e); }
      })
      .on("broadcast", { event: "ice_candidate" }, async ({ payload }) => {
        if (!mR.current || !pcR.current) return;
        if (pcR.current.remoteDescription) { try { await pcR.current.addIceCandidate(new RTCIceCandidate(payload.candidate)); } catch (_) {} }
        else { pIce.current.push(payload.candidate); }
      })
      .on("broadcast", { event: "call_ended" }, () => {
        if (!mR.current) return;
        secsR.current = secs; setCallDur(secsR.current); setEndR("ended"); setStage("ended"); destroyPC();
      })
      .on("broadcast", { event: "participant_state" }, ({ payload }) => {
        if (!mR.current) return;
        setParts(prev => prev.map(pt => pt.id === payload.userId ? { ...pt, muted: payload.muted, camOff: payload.camOff } : pt));
      })
      .subscribe();
    sigR.current = ch;
    return ch;
  ...
  // [FIX-INVITE-CH] Send invites via MessageNotificationService
  // which now sends to "incoming_calls:{calleeId}" — matching the listener
  const sendInvites = useCallback(async (callId) => {
    if (inviteSent.current || !call.outgoing) return;
    inviteSent.current = true;
    const callerName     = localUser.full_name;
    const callerAvatarId = localUser.avatar_id || null;
    const callType       = call.type || "audio";

    if (isGroup && Array.isArray(call.participants) && call.participants.length > 0) {
      await Promise.allSettled(
        call.participants
          .filter(pt => pt?.id && pt.id !== localUser.id)
          .map(pt => MessageNotificationService.sendCallInvite({
            callId, calleeId: pt.id, callerName, callerAvatarId, callType,
            groupName: call.name || "Group Call", participantCount: call.participants.length,
          }))
      );
    } else if (call.user?.id) {
      await MessageNotificationService.sendCallInvite({
        callId, calleeId: call.user.id, callerName, callerAvatarId, callType,
      });
    }
  }, [call, isGroup, localUser]);

  useEffect(() => {
    mR.current = true;
    // [FIX-CALLEE-STAGE] Use the callId passed from the invite so both sides
    // join the SAME signalling channel
    const callId = call.callId || call.id || `call_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
    (async () => {
      if (call.outgoing === true) {
        // CALLER: get media, build PC, subscribe to sig channel, then send invites
        ...
        // CALLEE: subscribe to sig channel first, then get media
        buildCh(callId);
        await getMedia();
      }
    })();
    return () => {
      mR.current = false;
      clearInterval(tiR.current); clearInterval(nqR.current);
      destroyPC();
      if (sigR.current) { supabase.removeChannel(sigR.current); sigR.current = null; }
    };
  }, []); // eslint-disable-line
  ...
  useEffect(() => {
    ...
    const t = setInterval(() => {
      if (!mR.current) return;
      const ids = new Set();
      if (Math.random() > .55) ids.add("local");
      parts.forEach(pt => { if (Math.random() > .65) ids.add(pt.id); });
      setSpkIds(ids);
    }, 1100);
    return () => clearInterval(t);
  }, [stage, parts]);
  ...
  const handleAccept = useCallback(async () => {
    if (!mR.current) return;
    setStage("conn");
    const s  = lsR.current || await getMedia();
    const pc = buildPC();
    if (s) addTracks(pc, s);
...
.dm-bars{display:flex;align-items:flex-end;gap:2px;height:16px;}.dm-bar{width:3px;border-radius:1px;}
.dm-nums{display:flex;flex-direction:column;}.dm-used{font-size:10px;font-weight:700;color:#fff;}.dm-rate{font-size:8px;color:rgba(255,255,255,.4);}
...
.aviz-b{width:3px;border-radius:2px;height:3px;}.aviz-b.aviz-on{animation:avizP .8s ease-in-out infinite alternate;}
...
.gg1{grid-template-columns:1fr;grid-template-rows:1fr;}.gg2{grid-template-columns:1fr 1fr;grid-template-rows:1fr;}.gg3{grid-template-columns:1fr 1fr;grid-template-rows:1fr 1fr;}.gg3 .ptile:nth-child(3){grid-column:1/-1;max-width:50%;justify-self:center;min-height:140px;}.gg4{grid-template-columns:1fr 1fr;grid-template-rows:1fr 1fr;}.gg6{grid-template-columns:1fr 1fr 1fr;grid-template-rows:1fr 1fr;}.ggn{grid-template-columns:repeat(3,1fr);overflow-y:auto;}
.ptile{position:relative;border-radius:18px;overflow:hidden;background:linear-gradient(145deg,rgba(18,18,18,.9),rgba(10,10,10,.95));border:1.5px solid rgba(255,255,255,.07);cursor:pointer;transition:border-color .25s,box-shadow .25s,transform .15s;min-height:90px;display:flex;align-items:center;justify-content:center;box-shadow:0 4px 24px rgba(0,0,0,.5);}
.ptile::before{content:'';position:absolute;inset:0;border-radius:18px;background:linear-gradient(145deg,rgba(255,255,255,.04),transparent);pointer-events:none;z-index:0;}
.ptile:hover{border-color:rgba(132,204,22,.35);transform:scale(1.012);}
.ptile.ptile-speak{border-color:#84cc16;box-shadow:0 0 0 2px rgba(132,204,22,.25),0 8px 40px rgba(132,204,22,.15),0 4px 24px rgba(0,0,0,.5);}
.ptile-ring{position:absolute;inset:-2px;border-radius:20px;border:2px solid #84cc16;animation:ptRing 1.2s ease-in-out infinite;pointer-events:none;z-index:5;}
...
.ptile-avimg{width:64px;height:64px;border-radius:50%;object-fit:cover;border:2.5px solid rgba(132,204,22,.3);box-shadow:0 4px 16px rgba(0,0,0,.5);}
...
.ptile-wave span:nth-child(1){height:8px;}.ptile-wave span:nth-child(2){height:18px;animation-delay:.08s;}.ptile-wave span:nth-child(3){height:28px;animation-delay:.16s;}.ptile-wave span:nth-child(4){height:18px;animation-delay:.08s;}.ptile-wave span:nth-child(5){height:8px;}
@keyframes ptWave{from{opacity:.25;transform:scaleY(.4)}to{opacity:1;transform:scaleY(1)}}
.ptile-foot{position:absolute;bottom:0;left:0;right:0;padding:24px 10px 8px;background:linear-gradient(to top,rgba(0,0,0,.85) 0%,rgba(0,0,0,.4) 60%,transparent 100%);display:flex;align-items:center;justify-content:space-between;z-index:2;}
