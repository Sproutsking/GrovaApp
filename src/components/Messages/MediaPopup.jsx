 ...
 * MediaPopup
 *
 * Renders a small toolbar of 4 buttons (Emoji, GIF, Meme, File).
 * Clicking one opens the corresponding panel.
 *
 * POSITIONING LOGIC:
 *   The popup toolbar AND any open sub-panel are positioned so they
 *   never escape the viewport.  Rules:
 *     - Vertical:  if the click point (triggerY) is more than 20px below
 *                  the screen midpoint → panel opens ABOVE the toolbar.
 *                  If more than 20px above midpoint → opens BELOW.
 *                  Within ±20px of midpoint → opens above (default).
 *     - Horizontal: if triggerX is in the right half → panel opens to the LEFT.
 *                   If in the left half → opens to the RIGHT.
 *     - Final clamp: after computing the ideal position we clamp all edges
 *                    to [8px, viewport - 8px] so nothing is ever cut off.
 */
