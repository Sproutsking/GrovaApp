...
// UPDATED: Withdraw button added after Receive in the primary action row.
// FIX preserved: EP always renders as integer matching BalanceCard.
import React, { useState, useEffect, useRef } from "react";
import {
  ArrowUpRight, Download, ArrowDownLeft, MoreHorizontal,
  TrendingUp, Repeat, Settings, Wifi, Coins, Zap,
  ChevronRight, Flame, Clock, ArrowUpToLine,
} from "lucide-react";
  ...
  return (
    <div style={{ position: "relative", flexShrink: 0, width: 40, height: 40 }}>
      ...
      <div className={`ov-tx-dir-badge ${dirClass}`}>
        {isPending ? <Clock size={7} color="#000" /> : isReceived ? (
          <svg width="8" height="8" viewBox="0 0 10 10" fill="none"><path d="M8 2 L2 8 M2 8 H6 M2 8 V4" stroke="#000" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>
        ) : (
          <svg width="8" height="8" viewBox="0 0 10 10" fill="none"><path d="M2 8 L8 2 M8 2 H4 M8 2 V6" stroke="#000" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg>
        )}
      </div>
    </div>
  );
...
function useAnimatedNumber(target, loading, duration = 700) {
  ...
  useEffect(() => {
    if (prevLoadingRef.current === true && loading === false) { fromRef.current = 0; setDisplay(0); }
    prevLoadingRef.current = loading;
    if (loading) { setDisplay(0); return; }
    const from = fromRef.current; const to = target;
    cancelAnimationFrame(rafRef.current); startRef.current = null;
    const ease = (t) => t < 0.5 ? 4 * t * t * t : 1 - Math.pow(-2 * t + 2, 3) / 2;
    rafRef.current = requestAnimationFrame(function tick(ts) {
      if (!startRef.current) startRef.current = ts;
      const elapsed = ts - startRef.current; const progress = Math.min(elapsed / duration, 1);
      const current = from + (to - from) * ease(progress); setDisplay(current);
      if (progress < 1) { rafRef.current = requestAnimationFrame(tick); } else { fromRef.current = to; setDisplay(to); }
    });
...
function DeltaFlash({ delta, currency }) {
  ...
  useEffect(() => {
    if (delta === 0) return;
    setVal(delta); setVisible(true); clearTimeout(timerRef.current);
  ...
  const isPos = val > 0;
  return (
    <div style={{ position: "absolute", top: -22, right: 0, padding: "3px 9px", borderRadius: 100, fontSize: 11, fontWeight: 800, fontFamily: "var(--font-mono,'DM Mono',monospace)", color: isPos ? "#a3e635" : "#f87171", background: isPos ? "rgba(163,230,53,0.12)" : "rgba(248,113,113,0.1)", border: `1px solid ${isPos ? "rgba(163,230,53,0.3)" : "rgba(248,113,113,0.2)"}`, pointerEvents: "none", animation: "ovDeltaIn 0.35s cubic-bezier(.34,1.56,.64,1) forwards", whiteSpace: "nowrap" }}>
...
function TxDirectionChip({ isIn, isPending }) {
  if (isPending) return <div style={{ width: 36, height: 36, borderRadius: 10, flexShrink: 0, background: "rgba(245,158,11,0.1)", border: "1px solid rgba(245,158,11,0.2)", display: "flex", alignItems: "center", justifyContent: "center" }}><Clock size={15} color="#f59e0b" /></div>;
  return <div style={{ width: 36, height: 36, borderRadius: 10, flexShrink: 0, background: isIn ? "rgba(163,230,53,0.08)" : "rgba(248,113,113,0.08)", border: `1px solid ${isIn ? "rgba(163,230,53,0.2)" : "rgba(248,113,113,0.2)"}`, display: "flex", alignItems: "center", justifyContent: "center" }}><svg width="16" height="16" viewBox="0 0 16 16" fill="none" style={{ transform: isIn ? "rotate(135deg)" : "rotate(-45deg)", color: isIn ? "#a3e635" : "#f87171" }}><path d="M3 13 L13 3 M13 3 H7 M13 3 V9" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" /></svg></div>;
}
...
const CSS = `
  ...
  .ov-actions{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:10px;}
  .ov-act-btn{display:flex;flex-direction:column;align-items:center;gap:7px;padding:14px 8px;border-radius:14px;border:1px solid rgba(255,255,255,0.07);background:rgba(255,255,255,0.03);cursor:pointer;transition:all .18s;color:rgba(255,255,255,0.55);font-size:11.5px;font-weight:600;font-family:inherit;}
  ...
  .ov-act-btn.withdraw{background:rgba(248,113,113,0.07);border-color:rgba(248,113,113,0.2);color:#f87171;}
  .ov-act-btn.withdraw:hover{background:rgba(248,113,113,0.12);border-color:rgba(248,113,113,0.35);}
  .ov-act-btn.more-toggle{margin-top:6px;width:100%;flex-direction:row;justify-content:center;}
  ...
  .ov-more-panel{display:grid;grid-template-columns:repeat(4,1fr);gap:8px;margin-top:8px;animation:moreIn .2s cubic-bezier(.34,1.56,.64,1);}
...
function AnimatedBalance({ value, loading, className, style, isInteger = false }) {
  ...
  const display = isInteger ? Math.round(animated).toLocaleString() : animated.toLocaleString(undefined, { maximumFractionDigits: 4 });
  return <div className={className} style={style}>{loading ? "—" : display}</div>;
}
...
function AssetCard({ type, icon, name, desc, value, fiatLine, chip, loading, delta, currency, isInteger }) {
  ...
  return (
    <div className={`ov-asset-card ${type} ${flashClass}`}>
      ...
      <div><div className="ov-asset-name">{name}</div><div className="ov-asset-desc">{desc}</div></div>
      <div className="ov-asset-right">
        <DeltaFlash delta={delta} currency={currency} />
        <AnimatedBalance value={value} loading={loading} className={`ov-asset-amount ${type}`} isInteger={isInteger} />
...
export default function OverviewTab({ balance, setActiveTab, loading, transactions = [], userId, username, showPayWave, currentUser }) {
  const [showMore, setShowMore] = useState(false);
  const [hideBalance, setHide] = useState(false);
  const { format } = useCurrency();
  const xev = balance?.tokens ?? 0;
  const ep  = Math.floor(balance?.points ?? 0);
  const prevXEV = useRef(xev); const prevEP = useRef(ep);
  const [xevDelta, setXevDelta] = useState(0);
  const [epDelta,  setEpDelta]  = useState(0);
  useEffect(() => {
    if (!loading) {
      const dx = xev - prevXEV.current; const de = ep - prevEP.current;
      if (dx !== 0) setXevDelta(dx); if (de !== 0) setEpDelta(de);
      prevXEV.current = xev; prevEP.current = ep;
    }
  }, [xev, ep, loading]);
  ...
  return (
    <div className="view-enter">
      <style>{CSS}</style>
      <BalanceCard balance={balance} loading={loading} hideBalance={hideBalance} onToggleHide={() => setHide(h => !h)} username={username} />
      ...
      <div className="ov-actions">
        ...
        {/* ── WITHDRAW — new, after Receive ── */}
        <button
          className="ov-act-btn withdraw"
          onClick={() => setActiveTab("withdraw")}
          title="Withdraw EP — tiered processing fees"
        >
          <div className="ov-act-icon"><ArrowUpToLine size={18} /></div>
          Withdraw
        </button>
      </div>

      {/* More toggle */}
      <button className="ov-act-btn more-toggle" onClick={() => setShowMore(m => !m)} style={{ width: "100%", marginTop: 6, flexDirection: "row", justifyContent: "center", gap: 6 }}>
        <MoreHorizontal size={16} />{showMore ? "Less" : "More"}
      </button>
      ...
      <div className="ov-section-head">
        <div className="ov-section-line" /><span className="ov-section-title">Your Assets</span><div className="ov-section-line" />
      </div>

      <AssetCard type="xev" icon={<Coins size={20} />} name="$XEV Token" desc="Transferable · On-chain" value={xev} fiatLine={loading ? "—" : `≈ ${format(xev, false)}`} loading={loading} delta={xevDelta} currency="$XEV" />
      <AssetCard type="ep" icon={<Zap size={20} />} name="Engagement Points" desc="Platform currency · Withdraw any time" value={ep} chip="WITHDRAWABLE" loading={loading} delta={epDelta} currency="EP" isInteger={true} />

      {showPayWave && (
        <>
          <div className="ov-section-head" style={{ marginTop: 8 }}>
            <div className="ov-section-line" /><span className="ov-section-title">Quick Pay</span><div className="ov-section-line" />
          </div>
          <div className="ov-paywave-card" onClick={() => setActiveTab("paywave")} role="button" tabIndex={0}>
            ...
            <div><div className="ov-pw-name">PayWave</div><div className="ov-pw-desc">Send money · Zero fees · Instant</div></div>
      ...
      <div className="ov-section-head" style={{ marginTop: 24 }}>
        <div className="ov-section-line" /><span className="ov-section-title">Recent Activity</span><div className="ov-section-line" />
      </div>

      {loading ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>{[1,2,3].map(i => <div key={i} className="ov-skel" style={{ height: 60 }} />)}</div>
      ) : transactions.length === 0 ? (
        <div className="ov-empty"><Zap size={32} className="ov-empty-icon" /><div className="ov-empty-title">No activity yet</div><div className="ov-empty-sub">Your transactions will appear here</div></div>
      ) : (
        <div className="ov-tx-list">
          {transactions.map((tx, idx) => {
            const isPending = tx._optimistic === true; const isSent = tx.change_type === "debit"; const isReceived = tx.change_type === "credit";
            const currency = tx.displayCurrency || tx.metadata?.currency || "EP"; const label = tx.displayLabel || (isSent ? "Sent" : "Received");
            const cp = tx.counterparty;
            const dateStr = tx.created_at ? new Date(tx.created_at).toLocaleString("en-NG", { month: "short", day: "numeric", hour: "2-digit", minute: "2-digit" }) : "Just now";
            const dirClass = isPending ? "pending-badge" : isReceived ? "in" : "out"; const amtClass = isReceived ? "in" : "out";
            return (
              <div key={tx.id || `opt-${idx}`} className={`ov-tx-row${isPending ? " pending" : ""}`} style={{ animationDelay: `${idx * 30}ms` }}>
                {cp ? <TxAvatar cp={cp} currentUser={currentUser} dirClass={dirClass} isPending={isPending} isReceived={isReceived} /> : <div className="ov-tx-avatar-wrap"><TxDirectionChip isIn={isReceived} isPending={isPending} /></div>}
                <div style={{ flex: 1, minWidth: 0 }}>
                  <div className="ov-tx-label">{label}{isPending && <span className="ov-pending-tag">SENDING</span>}</div>
                  {cp && <div className="ov-tx-counterparty">{isSent ? "to" : "from"} @{cp.username}</div>}
                  {tx.note && <div className="ov-tx-sub" style={{ fontStyle: "italic" }}>"{tx.note}"</div>}
                  <div className="ov-tx-sub">{isPending ? "Processing…" : dateStr}</div>
                </div>
                <div className="ov-tx-right">
                  <div className={`ov-tx-amount ${amtClass}`}>{isReceived ? "+" : "−"}{(tx.amount || 0).toLocaleString()} <span style={{ fontSize: 10, opacity: 0.7 }}>{currency}</span></div>
                  <div className={`ov-tx-dir-pill ${amtClass}`}>
                    {isReceived ? <svg width="7" height="7" viewBox="0 0 10 10" fill="none"><path d="M8 2L2 8M2 8H6M2 8V4" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg> : <svg width="7" height="7" viewBox="0 0 10 10" fill="none"><path d="M2 8L8 2M8 2H4M8 2V6" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>}
                    {isReceived ? "IN" : "OUT"}
                  </div>
                </div>
              </div>
      ...
      <div className="ov-burn-strip">
        ...
        <span>
          EP withdrawal tiers: <strong style={{ color: "rgba(248,113,113,.85)" }}>4%</strong> (100–1k EP) ·{" "}
          <strong style={{ color: "rgba(251,191,36,.85)" }}>3%</strong> (1k–10k EP) ·{" "}
          <strong style={{ color: "rgba(52,211,153,.85)" }}>2%</strong> (10k+ EP). Higher = lower fee + faster.
        </span>
      </div>
    </div>
  );
