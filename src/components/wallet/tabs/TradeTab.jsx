...
import { 
  ...
  Award, Calendar, MessageSquare as SupportIcon, Search, 
  Coins as AssetIcon, CreditCard as PaymentIcon,
} from 'lucide-react';
      ...
      <div className="profile-modal-header">
        ...
        <div className="profile-modal-info">
          <div className="profile-name-badge">
            <h3>{name}</h3>
            {profile.verified && <Shield size={15} className="verified-badge" />}
          </div>
...
const TradeTab = ({ setActiveTab }) => {
  ...
  const [yourAds, setYourAds] = useState([
    { id: 1, type: 'sell', amount: 200, price: 2.6, currency: 'NGN', method: 'Bank Transfer', min: 100, max: 500, status: 'active', terms: '' },
  ]);
  ...
  const [createForm, setCreateForm] = useState({
    type: 'sell', amount: '', price: '', currency: 'NGN', method: 'Bank Transfer', min: '', max: '', status: 'active', terms: ''
  });
  ...
  const paymentMethods = ['Bank Transfer', 'Mobile Money', 'PayPal', 'Crypto Wallet', 'Cash App', 'Venmo', 'Zelle'];
  ...
  const handleConfirm = (asBuyer) => {
    const trade = {
      id: Date.now(), type: subTab, amount: parseFloat(tradeAmount),
      price: selectedOffer.price, currency: selectedOffer.currency,
    ...
    setTradeAmount('');
    alert(`${asBuyer ? 'Payment confirmed' : 'Funds released'}. Trade completed!`);
  };
  ...
  const handleCreateOrEditAd = () => {
    ...
    const ad = {
      ...createForm, id: editAd ? editAd.id : Date.now(),
      amount: parseFloat(createForm.amount), price: parseFloat(createForm.price),
      min: parseFloat(createForm.min), max: parseFloat(createForm.max)
    };
    if (editAd) {
    ...
    setCreateForm({ type: 'sell', amount: '', price: '', currency: 'NGN', method: 'Bank Transfer', min: '', max: '', status: 'active', terms: '' });
    setEditAd(null);
    setShowCreateAd(false);
  };

  const handleDeleteAd = (id) => {
    if (window.confirm('Delete this ad?')) setYourAds(yourAds.filter((a) => a.id !== id));
  };
  ...
  useEffect(() => {
    const handleClickOutside = (event) => {
      if (assetFilterRef.current && !assetFilterRef.current.contains(event.target)) setAssetFilterOpen(false);
      if (paymentFilterRef.current && !paymentFilterRef.current.contains(event.target)) setPaymentFilterOpen(false);
      if (amountFilterRef.current && !amountFilterRef.current.contains(event.target)) setAmountFilterOpen(false);
    };
    document.addEventListener('mousedown', handleClickOutside);
    return () => document.removeEventListener('mousedown', handleClickOutside);
  }, []);

  const showFilters = (subTab === 'buy' || subTab === 'sell' || subTab === 'history') && !selectedOffer;
      ...
      <div className="trade-container">
        ...
        <div className="trade-header">
          ...
          <div className="header-left">
            <button onClick={() => setActiveTab('overview')} className="back-btn" title="Back">
              <ArrowLeft size={18} />
            </button>
          </div>
              ...
              <div className="trade-filters">
                <div className="filter-row">
                  ...
                  <div
                    ...
                    ref={assetFilterRef}
                    onClick={() => { setAssetFilterOpen(!assetFilterOpen); setPaymentFilterOpen(false); setAmountFilterOpen(false); }}
                      ...
                      <div className="filter-dropdown">
                        {['', 'NGN', 'USDT'].map((c) => (
                          <div
                            ...
                            onClick={(e) => { e.stopPropagation(); setFilterCurrency(c); setAssetFilterOpen(false); }}
                          >
                            {c === '' ? 'All Assets' : c}
                          </div>
                        ))}
                  ...
                  {/* Payment filter (buy/sell only) */}
                  {(subTab === 'buy' || subTab === 'sell') && (
                    <div
                      className={`filter-button ${filterMethod ? 'has-filter' : ''}`}
                      ref={paymentFilterRef}
                      onClick={() => { setPaymentFilterOpen(!paymentFilterOpen); setAssetFilterOpen(false); setAmountFilterOpen(false); }}
                      ...
                      {paymentFilterOpen && (
                        <div className="filter-dropdown">
                          <div
                            ...
                            onClick={(e) => { e.stopPropagation(); setFilterMethod(''); setPaymentFilterOpen(false); }}
                          >
                            All Methods
                          </div>
                          {paymentMethods.map((m) => (
                            <div
                              key={m}
                              className={`filter-option ${filterMethod === m ? 'active' : ''}`}
                              onClick={(e) => { e.stopPropagation(); setFilterMethod(m); setPaymentFilterOpen(false); }}
                            >
                              {m}
                            </div>
                          ))}
                  ...
                  {/* Amount filter (buy/sell only) */}
                  {(subTab === 'buy' || subTab === 'sell') && (
                    <div
                      className={`filter-button ${(filterMinAmount || filterMaxAmount) ? 'has-filter' : ''}`}
                      ref={amountFilterRef}
                      onClick={() => { setAmountFilterOpen(!amountFilterOpen); setAssetFilterOpen(false); setPaymentFilterOpen(false); }}
          ...
          <div className="header-right">
            <button onClick={() => setShowSupportModal(true)} className="support-btn" title="Support">
              <SupportIcon size={18} />
            </button>
          </div>
        </div>
        ...
        <div className="trade-quick-actions">
          {[
            ...
            <button
              ...
              onClick={() => { setSubTab(t.id); setSelectedOffer(null); setTradeInitiated(false); }}
            >
              {t.icon} {t.label}
            </button>
          ))}
        ...
        <div className="trade-scroll-area">
          ...
          {!selectedOffer && !tradeInitiated && (
            <div className="trade-content">
                      ...
                      filteredOffers(demoSellOffers).map((offer) => (
                        <div key={offer.id} className="trade-offer-card">
                          <div className="offer-header">
                            <div className="offer-avatar" onClick={(e) => { e.stopPropagation(); setShowProfileModal({ profile: offer.sellerDetails, name: offer.seller }); }}>
                              {offer.sellerDetails.avatar}
                            </div>
                            <div className="offer-user-info">
                              <div className="offer-user-name">
                                {offer.seller}
                                {offer.sellerDetails.verified && <Shield size={13} className="verified-badge" />}
                                <span className={`status-indicator ${offer.status}`} />
                              </div>
                              <div className="offer-user-stats">
                                <div className="offer-stat"><Star size={11} fill="#fbbf24" stroke="#fbbf24" />{offer.sellerDetails.rating}%</div>
                                <div className="offer-stat">{offer.sellerDetails.trades} trades</div>
                                <div className="offer-stat"><Clock size={11} />{offer.sellerDetails.responseTime}</div>
                              </div>
                            </div>
              ...
              {subTab === 'sell' && (
                <div className="trade-section">
                  <h3 className="trade-section-title"><DollarSign size={16} /> Available Buy Offers</h3>
                  <div className="trade-offers-list">
                    {filteredOffers(demoBuyOffers).length > 0 ? (
                      filteredOffers(demoBuyOffers).map((offer) => (
                        <div key={offer.id} className="trade-offer-card">
                          <div className="offer-header">
                            <div className="offer-avatar" onClick={(e) => { e.stopPropagation(); setShowProfileModal({ profile: offer.buyerDetails, name: offer.buyer }); }}>
                              {offer.buyerDetails.avatar}
                            </div>
                            <div className="offer-user-info">
                              <div className="offer-user-name">
                                {offer.buyer}
                                {offer.buyerDetails.verified && <Shield size={13} className="verified-badge" />}
                                <span className={`status-indicator ${offer.status}`} />
                              </div>
                              <div className="offer-user-stats">
                                <div className="offer-stat"><Star size={11} fill="#fbbf24" stroke="#fbbf24" />{offer.buyerDetails.rating}%</div>
                                <div className="offer-stat">{offer.buyerDetails.trades} trades</div>
                                <div className="offer-stat"><Clock size={11} />{offer.buyerDetails.responseTime}</div>
                              </div>
                            </div>
                            <div className="offer-price-section">
                              <div className="offer-price">{offer.price} {offer.currency}</div>
                ...
                <>
                  <div className="profile-edit-section">
                    <div className="profile-edit-header">
                      <h3 className="trade-section-title"><User size={16} /> Your Trading Profile</h3>
                    ...
                    {!showEditProfile ? (
                      <div className="profile-display">
                        ...
                        <div className="profile-display-info">
                          ...
                          <div className="profile-display-stats">
                            <span><Star size={11} fill="#fbbf24" stroke="#fbbf24" /> {userProfile.rating}%</span>
                      ...
                      <div className="trade-form" style={{ marginTop: '12px' }}>
                        <div className="form-group"><label className="form-label">Display Name</label><input type="text" className="form-input" value={userProfile.name} onChange={(e) => setUserProfile({ ...userProfile, name: e.target.value })} /></div>
                        <div className="form-group"><label className="form-label">Avatar Initials</label><input type="text" className="form-input" value={userProfile.avatar} onChange={(e) => setUserProfile({ ...userProfile, avatar: e.target.value.slice(0, 2).toUpperCase() })} maxLength={2} /></div>
                        <div className="form-group"><label className="form-label">Bio</label><textarea className="form-textarea" value={userProfile.bio} onChange={(e) => setUserProfile({ ...userProfile, bio: e.target.value })} placeholder="Tell traders about yourself..." /></div>
                        <div className="form-row">
                          <div className="form-group"><label className="form-label">Email</label><input type="email" className="form-input" value={userProfile.email} onChange={(e) => setUserProfile({ ...userProfile, email: e.target.value })} /></div>
                          <div className="form-group"><label className="form-label">Phone</label><input type="tel" className="form-input" value={userProfile.phone} onChange={(e) => setUserProfile({ ...userProfile, phone: e.target.value })} /></div>
                        </div>
                        <div className="form-row">
                          <div className="form-group"><label className="form-label">Preferred Payment</label><select className="form-select" value={userProfile.preferredPayment} onChange={(e) => setUserProfile({ ...userProfile, preferredPayment: e.target.value })}>{paymentMethods.map((m) => <option key={m} value={m}>{m}</option>)}</select></div>
                          <div className="form-group"><label className="form-label">Trading Hours</label><input type="text" className="form-input" value={userProfile.tradingHours} onChange={(e) => setUserProfile({ ...userProfile, tradingHours: e.target.value })} /></div>
                        </div>
                        <div className="form-group"><label className="form-label">Languages</label><input type="text" className="form-input" value={userProfile.languages} onChange={(e) => setUserProfile({ ...userProfile, languages: e.target.value })} /></div>
                        <button className="btn-primary" onClick={handleUpdateProfile}>Save Profile</button>
                      </div>
                    )}
                  </div>

                  <div className="trade-section">
                    <h3 className="trade-section-title"><List size={16} /> Your Ads ({yourAds.length})</h3>
                    <div className="trade-offers-list">
                      {yourAds.map((ad) => (
                        <div key={ad.id} className="ad-card">
                          <div className="ad-header">
                            <div className={`ad-type ${ad.type}`}>
                              {ad.type === 'sell' ? <DollarSign size={14} /> : <ShoppingCart size={14} />}
                              {ad.type.toUpperCase()} AD
                            </div>
                            <div className="ad-actions">
                              <button className={`ad-status-toggle ${ad.status}`} onClick={() => handleToggleAdStatus(ad.id)}>
                                {ad.status === 'active' ? 'Active' : 'Inactive'}
                              </button>
                              <button className="icon-btn" onClick={() => handleEditAd(ad)}><Edit size={14} /></button>
                              <button className="icon-btn delete" onClick={() => handleDeleteAd(ad.id)}><Trash2 size={14} /></button>
                            </div>
                          </div>
                  ...
                  {!showCreateAd && !editAd && (
                    <div className="create-ad-section">
                      <button className="btn-create-ad" onClick={() => setShowCreateAd(true)}>
                        <span style={{ fontSize: '20px', lineHeight: 1 }}>+</span> Create New Ad
                      </button>
                    </div>
                  )}
                    ...
                    <div className="trade-section">
                      ...
                      <div className="trade-form">
                        ...
                        <div className="form-actions">
                          <button className="btn-primary" onClick={handleCreateOrEditAd}>{editAd ? 'Update Ad' : 'Create Ad'}</button>
                          <button className="btn-secondary" onClick={() => { setEditAd(null); setShowCreateAd(false); setCreateForm({ type: 'sell', amount: '', price: '', currency: 'NGN', method: 'Bank Transfer', min: '', max: '', status: 'active', terms: '' }); }}>Cancel</button>
                        </div>
                      </div>
                    </div>
                  )}
              ...
              {subTab === 'history' && (
                <div className="trade-section">
                  <h3 className="trade-section-title"><Clock size={16} /> Trade History</h3>
                  <div className="history-filter-buttons">
                    {['all', 'completed', 'in-progress', 'canceled'].map((f) => (
                      <button key={f} className={`history-filter-btn ${historyFilter === f ? 'active' : ''}`} onClick={() => setHistoryFilter(f)}>
          ...
          {selectedOffer && !tradeInitiated && (
            <div className="trade-summary-interface">
              ...
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                <button className="back-btn" onClick={() => setSelectedOffer(null)}><ArrowLeft size={18} /></button>
                <div style={{ flex: 1 }}>
                  <div className="view-title">Initiate Trade</div>
                  <div className="view-subtitle">Review and start trading</div>
                </div>
                <button className="back-btn" onClick={() => setChatOpen(true)}><MessageSquare size={18} /></button>
              </div>

              {/* Counterparty card — clickable for profile */}
              <div className="summary-card" onClick={() => setShowProfileModal({ profile: counterpartyDetails, name: counterpartyName })} style={{ cursor: 'pointer' }}>
                <div className="offer-header" style={{ padding: 0 }}>
                  <div className="offer-avatar" style={{ pointerEvents: 'none' }}>{counterpartyDetails?.avatar}</div>
                  <div className="offer-user-info">
                    <div className="offer-user-name">
                      {counterpartyName}
                      {counterpartyDetails?.verified && <Shield size={13} className="verified-badge" />}
                    </div>
                    <div className="offer-user-stats">
                      <div className="offer-stat"><Star size={11} fill="#fbbf24" stroke="#fbbf24" />{counterpartyDetails?.rating}%</div>
                      <div className="offer-stat">{counterpartyDetails?.trades} trades</div>
                      <div className="offer-stat"><Clock size={11} />{counterpartyDetails?.responseTime}</div>
                    </div>
                  </div>
                  <div style={{ fontSize: '11px', color: '#555', fontFamily: 'JetBrains Mono, monospace' }}>Tap to view profile →</div>
                </div>
              </div>
              ...
              <div className="summary-card">
                ...
                <div className="summary-details">
                  ...
                  {selectedOffer.terms && (
                    <div style={{ paddingTop: '8px', borderTop: '1px solid #1e1e1e', marginTop: '4px' }}>
                      <div className="summary-detail-label" style={{ marginBottom: '6px' }}>Terms</div>
                      <div style={{ fontSize: '13px', color: '#aaa', lineHeight: 1.5 }}>{selectedOffer.terms}</div>
                    </div>
                  )}
                </div>
              </div>
                ...
                {tradeAmount && !isNaN(parseFloat(tradeAmount)) && (
                  <div style={{ marginTop: '12px', display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: '15px' }}>
                      <span style={{ color: '#666', fontFamily: 'JetBrains Mono, monospace', fontSize: '12px' }}>Total Cost</span>
                      <span style={{ fontWeight: 800, color: '#84cc16', fontFamily: 'JetBrains Mono, monospace' }}>
              ...
              <button
                ...
                style={{ width: '100%', padding: '14px', fontSize: '15px', opacity: tradeAmount ? 1 : 0.4 }}
              >
                {subTab === 'buy' ? <><ShoppingCart size={16} /> Initiate Buy</> : <><DollarSign size={16} /> Initiate Sell</>}
              </button>
            </div>
          )}
          ...
          {tradeInitiated && selectedOffer && (
            <div className="trade-summary-interface">
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '12px' }}>
                <button className="back-btn" onClick={() => { setTradeInitiated(false); setSelectedOffer(null); setTradeAmount(''); }}>
                  <ArrowLeft size={18} />
                </button>
                <div style={{ flex: 1 }}>
                  <div className="view-title">Ongoing Trade</div>
                  <div className="view-subtitle">Complete the transaction</div>
                </div>
                <button className="back-btn" onClick={() => setChatOpen(true)}><MessageSquare size={18} /></button>
              </div>

              <div className="summary-card" style={{ background: 'rgba(132, 204, 22, 0.05)', borderColor: 'rgba(132, 204, 22, 0.2)' }}>
                ...
                <div className="summary-details">
                  ...
                  <div className="summary-detail"><span className="summary-detail-label">Method</span><span className="summary-detail-value">{selectedOffer.method}</span></div>
                </div>
              </div>

              <div className="summary-card">
                <div className="summary-title">{subTab === 'buy' ? 'Buyer Instructions' : 'Seller Instructions'}</div>
                <div style={{ fontSize: '13px', color: '#ccc', lineHeight: 1.8, fontFamily: 'Syne, sans-serif' }}>
                  {subTab === 'buy' ? (
                    <>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                {subTab === 'buy' ? (
                  <button className="btn-buy" onClick={() => handleConfirm(true)} style={{ width: '100%', padding: '14px', fontSize: '14px' }}>
        ...
        {chatOpen && (
          <div className="trade-chat-modal">
            <div className="chat-header">
              <button className="back-btn" onClick={() => setChatOpen(false)}><ArrowLeft size={18} /></button>
              <div className="chat-user-info">
                <div className="chat-user-name">{counterpartyName}</div>
                <div style={{ display: 'flex', gap: '10px', fontSize: '11px', color: '#555', fontFamily: 'JetBrains Mono, monospace' }}>
                  <span><Star size={11} fill="#fbbf24" stroke="#fbbf24" /> {counterpartyDetails?.rating}%</span>
        ...
        {showSupportModal && (
          <div className="support-modal">
            <div className="support-header">
              <button className="back-btn" onClick={() => setShowSupportModal(false)}><ArrowLeft size={18} /></button>
            ...
            <div className="support-messages-container">
              {supportMessages.length === 0 ? (
                <div style={{ textAlign: 'center', color: '#333', marginTop: '60px', fontFamily: 'Syne, sans-serif', fontSize: '14px' }}>
            ...
            <div className="support-input-container">
              ...
              />
              <button className="support-send-btn" onClick={handleSendSupportMessage}>Send</button>
            </div>
          </div>
        )}
      </div>
