...
const ProfileSection = ({ userId, onProfileUpdate, onSignOut, onNavigate, currentUser }) => {
  ...
  const loadProfileData = async () => {
    try {
      ...
      const { data: profileData, error: profileError } = await supabase
        .from("profiles")
        .select("id, full_name, username, avatar_id, bio, verified, is_pro, created_at, email, phone, phone_verified, show_email, show_phone, payment_status, subscription_tier, account_activated, engagement_points, boost_selections")
        .eq("id", userId)
        .maybeSingle();
  ...
  const handleEditSuccess = (updatedData) => {
    ...
    if (onProfileUpdate) {
      onProfileUpdate({
        id: profile.id, fullName: updatedData.fullName,
        username: updatedData.username, avatar: updatedData.avatar,
        verified: profile.verified, isPro: profile.isPro,
      });
    }
  };
  ...
  const boostTiers     = new Set(["silver","gold","diamond"]);
  const hasBoostedTier = boostTiers.has(profile.subscriptionTier);
  ...
  return (
    <>
      ...
      <div className="profile-section">
        ...
        {/* [B1] Header card — BoostProfileCard wraps for themed bg/frame/overlays */}
        <BoostProfileCard
