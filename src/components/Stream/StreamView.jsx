...
const StageGrid = ({
  ...
  // Stage area height: 45vh on desktop, 38vw on mobile (handled by parent clamp)
  // Responsive stage height:
  //   Mobile  (≤768px)  : 220px
  //   Tablet  (769-1100) : ~38vw  (~380px at 1000px wide)
  //   Desktop (>1100px)  : 380px cap
  // Computed as CSS string, used for both grid container height and gridAutoRows
  const stageHcss = "clamp(220px, 38vw, 380px)";
  // For grid row calculation we need a numeric approximation.
  // We measure from the DOM at render time via a ref; until measured use 280 as safe default.
  const stageH =
...
const ViewerRoom = ({ session, currentUser, onClose }) => {
  ...
  // Permission gate overlay (reuses PermissionGate inside full-screen shell)
      ...
      {isDesktop ? (
        <DesktopLiveShell
          left={
            ...
            >
              {/* Video + stage + on-stage controls only — chat moves to sidebar */}
              <div
                style={{
                  height: "clamp(220px,38vw,380px)",
                  flexShrink: 0,
...
const HostRoom = ({ mode, micOn, camOn, stream }) => (
  ...
  >
    ...
    {/* Live camera renders via srcObject in the parent video element — this div is the fallback */}
  </div>
);
...
// ── Draggable divider ─────────────────────────────────────────────────────────
// Lets the user drag the split between video and sidebar.
...
// Shows: stream stats, live viewers, active live streams, chat/stage panels
const DesktopSidebar = ({
  ...
  useEffect(() => {
    // Fetch other live streams for the "Live Now" panel
    supabase
      .from("live_sessions")
...
// ── Desktop live shell — wraps video+controls on left, sidebar on right ────────
// Only used on desktop (>768px). Mobile keeps the original column layout.
// The draggable divider lets users resize the split.
const DesktopLiveShell = ({ left, sidebarProps }) => {
  const containerRef = useRef(null);
  const [leftW, setLeftW] = useState(65); // % of total width
  ...
  const onDrag = useCallback((clientX) => {
    ...
    setLeftW(Math.max(40, Math.min(80, pct))); // clamp 40–80%
  }, []);
...
const HostStreamView = ({ currentUser, userId, onClose, isSidebar }) => {
  const isDesktop = useIsDesktop(); // drives the two-panel layout
  const [mode, setMode] = useState("video");
  // phase: permission → preview → setup → connecting → live → ended
  ...
  const [showStageSearch, setShowStageSearch] = useState(false); // inline search panel in video area
  const [speakingId, setSpeakingId] = useState(null); // id of loudest speaker
  ...
  const isHost = true; // HostStreamView is always the host — used in StageGrid and stage tab
  ...
  // Broadcast stage state to all viewers
  useEffect(() => {
  ...
  // Speaking detection — AnalyserNode on host's own stream, broadcasts who is speaking
  useEffect(() => {
  ...
  // [A3] Track.enabled toggle (no reconnect)
  ...
  return (
    ...
    >
      ...
      {/* LIVE phase — two-panel on desktop, column on mobile */}
      {phase === "live" &&
        (() => {
          // The video+controls column (same on both mobile and desktop left panel)
          const videoColumn = (
            <div
              style={{
                display: "flex",
            ...
            >
              {/* ── STAGE AREA ──
              When solo (no co-participants): fixed 220px video like original.
              When people on stage: StageGrid takes over with proper grid layout.
              The grid is always the same height (260px) so controls never move.
          */}
              {allStage.length <= 1 ? (
                /* Solo host — responsive 16:9 */
                <div
                  style={{
                    height: "clamp(220px,38vw,380px)",
                    flexShrink: 0,
                ...
                >
                  ...
                  {/* + Add to stage — opens search panel overlay in this video area */}
                  {isHost && (
                    <>
                      <button
                ...
                /* Multi-person — adaptive StageGrid */
                <div style={{ position: "relative" }}>
                  <StageGrid
          ...
          // On desktop: two-panel layout with draggable divider + rich sidebar
          // On mobile: original column layout untouched
