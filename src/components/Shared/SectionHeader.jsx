...
const SectionHeader = ({
  ...
  return (
    <div className={`sh-root${className ? ` ${className}` : ""}`} style={style}>
      ...
      <style>{`
        .sh-root{display:flex;align-items:center;justify-content:space-between;gap:10px;padding-bottom:14px;border-bottom:1px solid var(--surface-border);margin-bottom:18px;position:relative;}
        ...
        .sh-icon{width:34px;height:34px;border-radius:12px;display:flex;align-items:center;justify-content:center;border:1px solid currentColor;flex-shrink:0;}
        .sh-text{display:flex;flex-direction:column;min-width:0;}
        .sh-title{font-size:14px;font-weight:800;color:var(--text);text-transform:uppercase;letter-spacing:0.02em;white-space:nowrap;}
        .sh-subtitle{font-size:12px;color:var(--text-secondary);margin-top:2px;line-height:1.3;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}
