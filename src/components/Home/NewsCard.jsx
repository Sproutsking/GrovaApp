// ============================================================================
// src/components/Home/NewsCard.jsx  — v4  REAL TIMESTAMPS
//
// CHANGES vs v3:
//  [T1] useRelTime() ticks every 15s — exported for use in NewsTab live cards.
//       Handles undefined/null/invalid dateStr gracefully (returns "").
//  [T2] LIVE NOW badge = published < 20 minutes ago (tighter than 30m).
//       Badge auto-transitions to "NEWS" without remount.
//  [T3] Proper timestamp display everywhere — no "just now" immortality.
//       If published_at is missing the card shows no timestamp, not "just now".
//  All design/reader/share logic unchanged.
// ============================================================================
...
// ── [T1] Live-ticking relTime hook ────────────────────────────────────────────
export function useRelTime(dateStr) {
  const calc = useCallback(() => {
    if (!dateStr) return "";
    const ts =
      typeof dateStr === "number" ? dateStr : new Date(dateStr).getTime();
...
// ── [T2] Live = published < 20 minutes ago ───────────────────────────────────
export function isLiveArticle(dateStr) {
  if (!dateStr) return false;
  const ts =
    typeof dateStr === "number" ? dateStr : new Date(dateStr).getTime();
  if (isNaN(ts)) return false;
  return Date.now() - ts < 20 * 60_000;
}
  ...
  const handleConfirm = async () => {
    ...
    try {
      window.dispatchEvent(
        new CustomEvent("grova:deductEP", {
          detail: {
            userId: currentUser.id,
            amount: NEWS_SHARE_COST_EP,
            reason: `Shared: ${title.slice(0, 60)}`,
            type: "news_share",
            newsId: post.id,
          },
        }),
      ...
      setErrMsg(err?.message || "Not enough EP.");
      setStep("error");
    }
  };
  ...
  const shareTargets = [
    {
      label: "WhatsApp",
      icon: "💬",
      href: `https://wa.me/?text=${encodeURIComponent(`${title} ${article_url}`)}`,
    },
    {
      label: "Twitter/X",
      icon: "🐦",
      href: `https://twitter.com/intent/tweet?text=${encodeURIComponent(title)}&url=${encodeURIComponent(article_url)}`,
    },
    {
      label: "Telegram",
      icon: "✈️",
      href: `https://t.me/share/url?url=${encodeURIComponent(article_url)}&text=${encodeURIComponent(title)}`,
    },
    {
      label: "LinkedIn",
      icon: "💼",
      href: `https://linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(article_url)}`,
    },
  ];
        ...
        <div className="nsm-sheet" onClick={(e) => e.stopPropagation()}>
          ...
          {step === "confirm" && (
            <>
              ...
              <p className="nsm-sub">
                Sharing news costs{" "}
                <strong className="nsm-ep">{NEWS_SHARE_COST_EP} EP</strong>
              </p>
              ...
              <div className="nsm-ep-cost">
                ...
                <span>{NEWS_SHARE_COST_EP} EP deducted from your wallet</span>
              </div>
              <div className="nsm-actions">
                <button className="nsm-btn nsm-btn-cancel" onClick={onClose}>
                  Cancel
                </button>
                <button
          ...
          {step === "done" && (
            <>
              ...
              {navigator.share && (
                <button
                  className="nsm-native-share"
                  onClick={async () => {
                    try {
                      await navigator.share({
                        title,
                        text: `${title} — via Xeevia`,
                        url: article_url,
                      });
                    } catch {
                      handleCopy();
...
const FullScreenNewsView = ({ post, currentUser, onClose }) => {
  ...
  const live = isLiveArticle(published_at);
  ...
  const displayTxt = fetchState === "done" && body ? body : description || null;
  ...
  return ReactDOM.createPortal(
    <>
      <div className="fsnv-root" role="dialog" aria-modal="true">
        ...
        <div
          className={isMobile ? "fsnv-sheet" : "fsnv-desktop-card"}
        ...
        >
          ...
          <div className="fsnv-topbar">
            ...
            <div className="fsnv-topbar-info">
              ...
              {published_at && (
                <span className="fsnv-topbar-date">
            ...
            <div style={{ flex: 1 }} />
            {!isMobile && isFullText && (
              <span className="fsnv-badge fsnv-badge-full">
            ...
            {ts && (
              <div
                className={`fsnv-news-tag${live ? " fsnv-news-tag--live" : ""}`}
              >
                <span className="fsnv-live-dot" />
                {live ? "LIVE NOW" : ts}
              </div>
            )}
          </div>
          ...
          <div className="fsnv-body">
            {hasImage && (
              <div className="fsnv-hero-wrap">
                <img
                  src={image_url}
                  alt={title}
                  className="fsnv-hero-img"
            ...
            <div className="fsnv-content-wrap">
              ...
              <div className="fsnv-tags">
                ...
                {region && (
                  <span className="fsnv-tag fsnv-tag-neutral">
                    <Globe size={9} />
                    {region.toUpperCase()}
                  </span>
                )}
                {asset_tag && (
                  <span className="fsnv-tag fsnv-tag-crypto">
                    <Tag size={9} />
                    {asset_tag}
                  </span>
                )}
                {ts && (
                  <span className="fsnv-tag fsnv-tag-time">
                    <Clock size={9} />
                    {ts}
                  </span>
                )}
              ...
              <div className="fsnv-source-row">
                ...
                <div className="fsnv-source-info">
                  ...
                  {isFullText ? (
                    ...
                    fetchState !== "loading" && (
                      <span className="fsnv-badge fsnv-badge-excerpt">
                        <AlertCircle size={9} /> RSS excerpt
                      </span>
                    )
                  )}
                </div>
                <button
                  className="fsnv-share-btn"
                  onClick={() => setShowShare(true)}
                >
                  <Share2 size={14} />
                  <Zap size={10} />
                  {NEWS_SHARE_COST_EP}EP
                </button>
              </div>
              ...
              {fetchState === "loading" && (
                <div className="fsnv-loading">
                  <Loader size={18} className="fsnv-spin" />
                  <span>Loading full article…</span>
                </div>
              )}
              {fetchState !== "loading" && displayTxt && (
                <div className="fsnv-article-text">{displayTxt}</div>
              )}
              {fetchState === "failed" && !displayTxt && (
                <div className="fsnv-unavailable">
                  <AlertCircle size={24} />
                  <p>Visit the source to read this article.</p>
                </div>
              )}
              <div className="fsnv-attribution">
                <Rss size={12} />
      ...
      <div className="nc-card content-card">
        ...
        <div className="nc-header">
          ...
          <div className="nc-source-info">
            <div className="nc-source-row-top">
              ...
              <span className="nc-rss-icon">
                <Rss size={9} />
              </span>
            </div>
            <div className="nc-meta">
              {/* [T1] Accurate live-ticking timestamp */}
              {ts && (
                <span className="nc-ts">
                  <Clock size={10} />
                  {ts}
                </span>
              )}
          ...
          {/* [T2] LIVE NOW or NEWS badge — reflects actual time */}
          <div className={`nc-live-badge${live ? " nc-live-badge--live" : ""}`}>
        ...
        <div className="nc-body">
          ...
          {hasDesc && (
            <>
              <div
                ref={descRef}
                className={`nc-desc${!expanded && needsTruncate ? " nc-desc-clamp" : ""}`}
              >
                {description}
              </div>
              {needsTruncate && (
        ...
        {(category || region || asset_tag) && (
          <div className="nc-tags-row">
            ...
            {region && (
              <span className="nc-tag nc-tag-neutral">
                <Globe size={9} />
                {region.toUpperCase()}
              </span>
            )}
            {asset_tag && (
              <span className="nc-tag nc-tag-crypto">
                <Tag size={9} />
                {asset_tag}
              </span>
            )}
        ...
        <div className="nc-footer">
          <button className="nc-btn-read" onClick={open}>
            <BookOpen size={12} /> Read more
          </button>
          ...
          {article_url && (
            <button className="nc-btn-src" onClick={openSrc}>
...
.nc-ts{display:flex;align-items:center;gap:3px;font-size:10.5px;color:rgba(255,255,255,0.35);font-weight:500;font-variant-numeric:tabular-nums;}
.nc-region{display:flex;align-items:center;gap:3px;font-size:10px;color:rgba(255,255,255,0.3);font-weight:600;}
.nc-live-badge{display:flex;align-items:center;gap:5px;padding:3px 8px;border-radius:999px;background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.18);font-size:9px;font-weight:800;color:#f87171;letter-spacing:0.08em;flex-shrink:0;white-space:nowrap;}
.nc-live-badge--live{background:rgba(239,68,68,0.15);border-color:rgba(239,68,68,0.45);color:#fca5a5;animation:ncLiveGlow 2s ease-in-out infinite;}
@keyframes ncLiveGlow{0%,100%{box-shadow:0 0 0 rgba(239,68,68,0)}50%{box-shadow:0 0 12px rgba(239,68,68,0.4)}}
...
.nsm-ep{color:#84cc16;}
.nsm-article-preview{width:100%;background:rgba(255,255,255,0.04);border:1px solid rgba(255,255,255,0.08);border-radius:12px;padding:12px 14px;}
...
.fsnv-backdrop{position:absolute;inset:0;background:rgba(0,0,0,0.84);backdrop-filter:blur(6px);animation:fsnvFade 0.2s ease both;}
...
.fsnv-news-tag{display:flex;align-items:center;gap:4px;padding:3px 8px;border-radius:999px;background:rgba(239,68,68,0.08);border:1px solid rgba(239,68,68,0.18);font-size:9px;font-weight:800;color:#f87171;letter-spacing:0.08em;flex-shrink:0;white-space:nowrap;}
.fsnv-news-tag--live{background:rgba(239,68,68,0.15);border-color:rgba(239,68,68,0.45);color:#fca5a5;animation:ncLiveGlow 2s ease-in-out infinite;}
...
@media(max-width:768px){.fsnv-hero-wrap{height:200px;}.fsnv-hero-img{height:200px;}.fsnv-content-wrap{padding:14px 16px 10px;}.fsnv-title{font-size:17px;}.fsnv-article-text{font-size:15px;}}
`;
