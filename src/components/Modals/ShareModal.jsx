...
const ShareModal = ({
  ...
  const handleSendDMs = async () => {
    ...
    try {
      ...
      const shareLabel = contentType === "reel" ? "a reel" : contentType === "story" ? "a story" : "a post";
      const shareLink = shareMessage
        ? `${senderName} shared ${shareLabel}\n${shareUrl}\n\n${shareMessage}`
        : `${senderName} shared ${shareLabel}\n${shareUrl}`;
      ...
      try {
        if (contentType === "post")
          await postService.sharePost?.(content.id, "direct");
        else if (contentType === "reel")
          await reelService.shareReel?.(content.id, "direct");
        else if (contentType === "story")
          await storyService.shareStory?.(content.id, "direct");
      } catch (e) {
        /* non-blocking */
      }
      ...
      setSent(true);
      showToast(
        `Sent to ${selectedUsers.length} person${selectedUsers.length > 1 ? "s" : ""}!`,
      );
  ...
  const shareExternal = async (platform) => {
    ...
    try {
      if (contentType === "post")
        await postService.sharePost?.(content.id, "external");
      else if (contentType === "reel")
        await reelService.shareReel?.(content.id, "external");
      else if (contentType === "story")
        await storyService.shareStory?.(content.id, "external");
    } catch (e) {
      /* non-blocking */
    }
  };
  ...
  return (
    <>
      ...
      <div className="sm-modal" onClick={(e) => e.stopPropagation()}>
        ...
        {toast && (
          <div
            className={`sm-toast ${toast.type === "error" ? "sm-toast-error" : ""}`}
          >
            ...
            <div className="sm-contacts-grid">
              {displayUsers.map((profile) => (
                ...
                >
                  ...
                  <span className="sm-contact-username">
                    @{profile.username}
                  </span>
                </button>
              ))}
        ...
        <div className="sm-external-grid">
          ...
          <button
            className="sm-ext-btn sm-whatsapp"
          ...
          <button
            className="sm-ext-btn sm-twitter"
          ...
          <button
            className="sm-ext-btn sm-facebook"
          ...
          <button
            className="sm-ext-btn sm-telegram"
          ...
          {typeof navigator !== "undefined" && navigator.share ? (
            <button
              className="sm-ext-btn sm-native"
            ...
            <button
              className="sm-ext-btn sm-linkedin"
