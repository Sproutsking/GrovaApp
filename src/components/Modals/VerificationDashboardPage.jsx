  ...
  const renderSectionPage = () => (
    <div className={`vdp-section-page vdp-section-${selected.id}`} style={{ "--section-accent": selected.accent || "#84cc16" }}>
      ...
      <section className="vdp-records">
        ...
        {selected.items.length === 0 ? <div className="vdp-empty"><span className="vdp-empty-seal">◇</span><span>No verified evidence found for this section yet.</span><small>This chamber is ready for its first proof record.</small></div> : <div className="vdp-evidence">{selected.items.map((item, index) => <article className="vdp-evidence-item" key={item.id || item.title}><div className="vdp-evidence-seal" aria-hidden="true"><span>{item.verified ? "✓" : "·"}</span></div><div className="vdp-evidence-index">PROOF {String(index + 1).padStart(2, "0")}</div><div className="vdp-evidence-title"><span>{item.title}</span><span style={{color:item.verified ? "#84cc16" : "#9ca3af"}}>{item.verified ? "Verified" : "Tracked"}</span></div><div className="vdp-evidence-meta">{item.provider} · {item.evidence_type}</div>{item.summary && <div className="vdp-evidence-summary">{item.summary}</div>}<div className="vdp-evidence-foot"><span>Confidence {item.confidence || "medium"}</span><span>{item.verificationLevel || "standard"}</span></div></article>)}</div>}
      </section>
    </div>
  );
  ...
  return ReactDOM.createPortal(
    <div className={`vdp-root${selected ? " has-selection" : ""}`}>
      ...
      <style>{`
        ...
        @media(max-width:700px){.vdp-data-rain{opacity:.18}.vdp-evidence-item{padding-left:58px}.vdp-evidence-foot{flex-wrap:wrap;gap:7px}.vdp-hero-grid{background-size:24px 24px}}
      `}</style>
      <style>{`
