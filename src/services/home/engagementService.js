...
const COMMENT_REVENUE_SPLIT = 0.4; // 40% to commenter
...
class EngagementService {
  ...
  async likeContent(contentType, contentId, userId) {
    try {
      ...
      if (existing) {
        ...
        await this.awardEngagementPoints(
          contentType,
          contentId,
          userId,
          "like",
  ...
  async awardEngagementPoints(
    ...
    try {
      ...
      const platformFee = isPro
        ? PLATFORM_FEES.PRO_USER
        : PLATFORM_FEES.FREE_USER;
  ...
  async awardCommentLikePoints(commentId, likingUserId) {
    try {
      ...
      const commenterPro =
        users?.find((u) => u.id === commenterId)?.is_pro || false;
      const ownerPro =
        users?.find((u) => u.id === contentOwnerId)?.is_pro || false;

      // Apply platform fees
      const commenterFee = commenterPro
        ? PLATFORM_FEES.PRO_USER
        : PLATFORM_FEES.FREE_USER;
      const ownerFee = ownerPro
        ? PLATFORM_FEES.PRO_USER
        : PLATFORM_FEES.FREE_USER;
