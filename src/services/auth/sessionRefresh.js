// ============================================================================
// src/services/auth/sessionRefresh.js  — v3  PRODUCTION-GRADE
//
...
//
// KEY FIX v3:
//   • initialize() is now called automatically on import (singleton boot)
//   • ensureValidSession() skips refresh entirely during OAuth callback
//     so it never interferes with the Google/Twitter/Facebook redirect flow
// ============================================================================
...
// ── Detect OAuth callback — NEVER interfere with the redirect exchange ────────
...
class SessionRefreshManager {
  ...
  async initialize() {
    ...
    try {
      ...
      // Don't touch the session if we're mid OAuth callback —
   ...
   *
   * KEY: If we're in the middle of an OAuth callback we return whatever
   * Supabase currently has WITHOUT triggering any refresh — the SDK is
   * already doing the exchange and we must not race it.
   */
  async getValidSession() {
    // During OAuth callback — hands off, let Supabase finish
    if (isOAuthCallback()) {
      try {
        const {
          data: { session },
    ...
    try {
      ...
      if (error) {
        ...
        if (isRevoked) {
          ...
          // We do NOT sign them out immediately. The next action that needs
          // a fresh token will detect null and surface a re-auth prompt.
          console.warn(
...
// Boot immediately but don't block the module
