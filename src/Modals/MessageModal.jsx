...
const MessageModal = ({ recipient, onClose, currentUser }) => {
  ...
  useEffect(() => {
    // Listen for DOM back registry close events
    const onDomClose = (e) => {
      onClose?.();
    };
  ...
  const handleSend = async () => {
    ...
    setSending(true);
    try {
      // Ensure a conversation exists between current user and recipient
      const userId = currentUser?.id;
      if (!userId) throw new Error("Not authenticated");

      const conv = await dmMessageService.createConversation(userId, recipient.id);
      if (!conv || !conv.id) throw new Error("Could not create conversation");
