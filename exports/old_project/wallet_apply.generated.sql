...
-- ENGAGEMENT POINTS TREASURY & ACCOUNTING
...
-- PAYMENT PROCESSING (Stripe, Paystack, Web3)
...
-- ============================================================================
-- WALLET PROJECT SCHEMA (wyqtcjqbdniwebvrwdnk)
-- Payments, Ledgers, Payouts, Treasury, Transactions
-- ============================================================================

-- MINIMAL PROFILES REFERENCE
...
-- PAYWAVE FEE CONFIG & PLATFORM SETTINGS
...
CREATE TABLE IF NOT EXISTS distribution_deep_links (
  id uuid DEFAULT gen_random_uuid(),
  post_id uuid,
...
CREATE TABLE IF NOT EXISTS ep_action_rate_limits (
  user_id uuid,
...
CREATE TABLE IF NOT EXISTS ep_treasury_disbursements (
  id uuid DEFAULT gen_random_uuid(),
...
CREATE TABLE IF NOT EXISTS ep_treasury_ledger (
  id uuid DEFAULT gen_random_uuid(),
  ...
  created_at timestamptz DEFAULT now() NOT NULL,
  PRIMARY KEY (id),
  FOREIGN KEY (ref_user_id) REFERENCES profiles(id),
  FOREIGN KEY (authorized_by) REFERENCES profiles(id)
);
...
-- Generated from schema_definitions.json for p2p_escrow
CREATE TABLE IF NOT EXISTS p2p_escrow (
  ...
  FOREIGN KEY (holder_id) REFERENCES profiles(id)
);
...
CREATE TABLE IF NOT EXISTS p2p_trade_messages (
  id uuid DEFAULT gen_random_uuid(),
  trade_id uuid,
  sender_id uuid,
  message text,
...
CREATE TABLE IF NOT EXISTS paywave_admin_audit (
  id uuid DEFAULT gen_random_uuid(),
  admin_id uuid,
  action text NOT NULL,
  target_type text,
...
CREATE TABLE IF NOT EXISTS paywave_transactions (
  id uuid DEFAULT gen_random_uuid(),
  user_id uuid,
  ...
  PRIMARY KEY (id),
  FOREIGN KEY (user_id) REFERENCES profiles(id),
  FOREIGN KEY (xrc_record_id) REFERENCES xrc_records(record_id)
);
...
CREATE TABLE IF NOT EXISTS web3_auto_payment_sessions (
  id uuid DEFAULT gen_random_uuid(),
  user_id uuid,
...
CREATE TABLE IF NOT EXISTS web3_pending_confirmations (
  id uuid DEFAULT gen_random_uuid(),
  payment_id uuid,
  user_id uuid,
  chain_type text NOT NULL,
...
CREATE TABLE IF NOT EXISTS web3_webhook_events (
  id uuid DEFAULT gen_random_uuid(),
  payment_id uuid,
  user_id uuid,
  chain_type text NOT NULL,
...
CREATE TABLE IF NOT EXISTS xrc_paywave_records (
  id uuid DEFAULT gen_random_uuid(),
  transaction_id uuid,
  xrc_record_id uuid,
  ...
  created_at timestamptz DEFAULT now(),
  PRIMARY KEY (id),
  FOREIGN KEY (transaction_id) REFERENCES paywave_transactions(id),
  FOREIGN KEY (xrc_record_id) REFERENCES xrc_records(record_id)
);
